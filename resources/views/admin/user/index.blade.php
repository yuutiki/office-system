...
<x-app-layout>
    <x-slot name="header">
        ...
        <div class="flex flex-row-reverse">
            <x-general-button class="mt-4" onclick="location.href='/user/create'">
                新規作成
            </x-general-button>
        </div>
        ...
        <form method="GET" action="{{ route('user.index') }}">   
            ...
            <label for="is_finished" class="block mt-2 mb-2 text-sm font-medium text-gray-900 dark:text-white"></label>
                <select  name="finish" value="0" class=" w-20 ml-2 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    ...
                    <option value="{{$user->is_finished}}">{{$user->is_finished}}</option>
                    @endforeach
                </select>
        </form>   
    </div>
            ...
            @foreach ($users as $user)
                <tbody>
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-600 dark:text-white">
                        <td scope="row" class="pl-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            {{$user->name}}
                        </td>
                        <td class="px-1 py-2">
                            {{$user->email}}
                        ...
                        @if($user->is_finished == "0")
                            <td class="px-2 py-2 text-fuchsia-300">
                                {{ $remaining }}
                            </td>
                        @else
                            <td class="px-2 py-2">
                        ...
                        <td class="px-4 py-2 flex justify-center items-center">
                            <form action="{{route('user.destroy',$user)}}" method="POST">
                                @csrf
                                @method('delete')
                                <button type="submit"  onClick="return confirm('本当に削除しますか？')" class="font-medium text-blue-600 dark:text-red-500 hover:underline">削除</button>
                            </form>
                        </td>
                    </tr>
                </tbody>
