<x-app-layout>
    <x-slot name="header">
        ...
        <div class="w-5/6 m-auto relative  rounded-t  shadow-md dark:bg-gray-700 dark:text-gray-900 bg-gray-50 mt-4">
            <!-- Start coding here -->
            <div class="relative bg-white dark:bg-gray-800">
                <div class="flex flex-col items-center justify-between p-4 space-y-3 md:flex-row md:space-y-0 md:space-x-4">
                    <div class="w-full md:w-1/2">
                        <form method="GET" action="{{ route('user.index') }}" id="link-search-form" class="flex items-center">
                            @csrf
                            ...
                            <input type="search" id="user_name" name="user_name" value="@if (isset($user_name)){{$user_name}}@endif" class="block w-full p-2 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="氏名">
                            </div>
                            ...
                            <div class="ml-2">
                                ...
                                <!-- Dropdown menu -->
                                <div id="filterDropdown" class="z-50 hidden w-56 p-3 bg-white rounded-e rounded-s shadow dark:bg-gray-600">
                                    ...
                                    <ul class="space-y-2 text-sm" aria-labelledby="dropdownDefault">
                                        ...
                                        <li class="flex items-center">
                                            <input id="employee-{{ $employeeStatus->id }}" type="checkbox" name="employeeStatuses[]" @if(in_array($employeeStatus->id, $selectedRoles)) checked @endif value="{{$employeeStatus->id}}" class="w-4 h-4 bg-gray-100 border-gray-300 rounded text-blue-600 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500" />
                            ...
                            <button type="button" value="reset" id="clear" form="link-search-form" class="p-2.5 ms-2 text-sm font-medium text-white bg-blue-700 rounded-lg border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                    ...
                    <div class="flex flex-col items-stretch justify-end flex-shrink-0 w-full space-y-2 md:w-auto md:flex-row md:space-y-0 md:items-center md:space-x-3">
                        <button type="button" onclick="location.href='{{ route('user.create') }}'" class="flex items-center justify-center px-4 py-2 text-sm font-medium text-white rounded-lg bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
    ...
    <div class="w-5/6 m-auto relative overflow-x-auto rounded-b shadow-md dark:bg-gray-700  dark:text-gray-900 bg-gray-300">
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                @foreach ($users as $user)
                    ...
                    {{-- 削除確認モーダル画面 End --}}
                    <!-- 更新drawer --> 
                    <div id="dupdateModal-{{$user->id}}" class="fixed top-0 right-0 z-50 h-screen p-4 overflow-y-auto transition-transform w-[30rem] translate-x-full bg-white  dark:bg-gray-800" tabindex="-1" aria-labelledby="dupdateModal-{{$user->id}}">
                        <div class="">
                        ...
                        <form id="updateForm-{{$user->id}}" method="POST" action="{{ route('user.update', $user->id) }}">
                            ...
                            {{-- <label class="relative inline-flex items-center cursor-pointer mt-4">
                                ...
                                @if(old('is_enabled_' . $user->id, $user->is_enabled) === 1)
                                    <input type="checkbox" name="is_enabled_{{$user->id}}" id="is_enabled-{{$user->id}}" value="1" class="sr-only peer" checked="checked">
                                @else
                                    <input type="checkbox" name="is_enabled_{{$user->id}}" id="is_enabled-{{$user->id}}" value="1" class="sr-only peer">
                                @endif
                            ...
                            <div class="w-full flex flex-col col-span-2 mt-4">
                                ...
                                <input type="text" maxlength="20" name="kana_name_{{$user->id}}" id="kana_name-{{$user->id}}" value="{{old('kana_name_' . $user->id, $user->kana_name)}}" class="dark:bg-white w-auto py-1 border border-gray-300 rounded-s rounded-e mt-1 mb-1" required>
                            </div>
                            ...
                            <div class="w-full flex flex-col">
                                <label for="employee_status_id" class="block mt-4 text-sm font-medium text-gray-900 dark:text-white">在職状態</label>
                                <select name="employee_status_id_{{$user->id}}" id="employee_status_id-{{$user->id}}" value="{{old('employee_status_id')}}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-s rounded-e focus:ring-primary-600 focus:border-primary-600 block w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required>
                                    @foreach($employeeStatuses as $employeeStatus)
                            ...
                            <div class="w-full flex flex-col">
                                <label for="role_id" class="block mt-4 text-sm font-medium text-gray-900 dark:text-white">権限</label>
                                <select name="role_id_{{$user->id}}" id="role_id-{{$user->id}}" value="{{old('role_id')}}" class="mt-1 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-s rounded-e focus:ring-primary-600 focus:border-primary-600 block w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required>
                                    @foreach($roles as $role)
                            ...
                            <div class="w-full flex flex-col">
                                <label for="company_id" class="block mt-4 text-sm font-medium text-gray-900 dark:text-white">所属1</label>
                                <select name="company_id_{{$user->id}}"  id="company_id-{{$user->id}}" value="{{old('company_id_' . $user->id)}}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-s rounded-e focus:ring-primary-600 focus:border-primary-600 block w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required>
                            ...
                            <div class="w-full flex flex-col">
                                <label for="department_id" class="block mt-4 text-sm font-medium text-gray-900 dark:text-white">所属2</label>
                                <select name="department_id_{{$user->id}}" id="department_id-{{$user->id}}" value="{{old('department_id_' . $user->id, $user->division_id)}}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-s rounded-e focus:ring-primary-600 focus:border-primary-600 block w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required>
                                    @foreach($departments as $department)
                                    <option value="{{ $department->id }}"  @selected($department->id == old('department_id_' . $user->id, $user->division_id))>{{ $department->department_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="w-full flex flex-col">
                                <label for="division_id" class="block mt-4 text-sm font-medium text-gray-900 dark:text-white">所属3</label>
                                <select name="division_id_{{$user->id}}" id="division_id-{{$user->id}}" value="{{old('division_id_' . $user->id, $user->division_id)}}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-s rounded-e focus:ring-primary-600 focus:border-primary-600 block w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" >
                            ...
                            <div class="grid grid-cols-2 gap-4 mt-4">
                                <button type="button" onclick="submitAndUpdateDrawer({{$user->id}})" class="w-full justify-center text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                                    Update
                                </button>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (!isValidationProcessed && drawerId) {
                ...
                // Drawerを表示
                document.getElementById('dupdateModal-' + drawerId).classList.remove('translate-x-full');
