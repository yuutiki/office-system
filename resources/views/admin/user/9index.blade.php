            ...
            <tbody>
                @foreach ($users as $user)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-600 dark:text-white dark:hover:bg-gray-600">
                        ...
                        <td class="px-1 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            <div class="text-center">
                                <button class="button-edit" type="button" data-drawer-target="dupdateModal-{{$user->id}}" data-drawer-show="dupdateModal-{{$user->id}}" data-drawer-placement="right" aria-controls="dupdateModal-{{$user->id}}">
                                編集
                                </button>
                            </div>
                        </td>
                        ...
                        <div class="relative w-full max-w-md max-h-full">
                            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                ...
                                <div class="p-6 text-center">
                                    ...
                                    <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">本当に削除しますか？</h3>
                                    <form action="{{route('link.destroy',$user->id)}}" method="POST" class="text-center m-auto">
                                        @csrf
                                        @method('delete')
                                        <button type="submit" data-modal-hide="deleteModal-{{$user->id}}" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2">
                                            削除
                    ...
                    <div id="dupdateModal-{{$user->id}}" class="fixed top-0 right-0 z-50 h-screen p-4 overflow-y-auto transition-transform w-[30rem] translate-x-full bg-white  dark:bg-gray-800" tabindex="-1" aria-labelledby="dupdateModal-{{$user->id}}">
                        ...
                        <form method="POST" action="{{ route('link.update', $user->id) }}">
                            @csrf
                            @method('PUT')
                            <div class="w-full flex flex-col col-span-2 mt-10">
                                <label for="display_name-{{$user->id}}" class="font-semibold dark:text-gray-100 text-gray-900 leading-none mt-1">表示名</label>
                                <input type="text" maxlength="20" name="display_name" id="-{{$user->id}}" value="{{old('display_name',$user->display_name)}}" class="dark:bg-white w-auto py-1 border border-gray-300 rounded-s rounded-e mt-1 mb-1" required>
                            </div>
                            @error('display_name')
                            ...
                            <div class="w-full flex flex-col col-span-2">
                                <label for="display_order-{{$user->id}}" class="font-semibold dark:text-gray-100 text-gray-900 leading-none mt-1">表示順</label>
                                <input type="number" min="1" max="99" name="display_order" id="display_order-{{$user->id}}" value="{{old('display_order',$user->display_order)}}" class="dark:bg-white w-auto py-1 border border-gray-300 rounded-s rounded-e mt-1 mb-1" required>
                            </div>
                            @error('display_order')
                              <div class="text-red-500">{{ $message }}</div>
                            @enderror
                            <div class="w-full flex flex-col col-span-2">
                                <label for="url-{{$user->id}}" class="font-semibold dark:text-gray-100 text-gray-900 leading-none mt-1">URL</label>
                                <input type="url" maxlength="100" name="url" id="url-{{$user->id}}" value="{{old('url',$user->url)}}" class="dark:bg-white w-auto py-1 border border-gray-300 rounded-s rounded-e mt-1 mb-1" required>
                            </div>
                            @error('url')
                              <div class="text-red-500">{{ $message }}</div>
                            @enderror
