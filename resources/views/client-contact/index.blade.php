<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between w-full whitespace-nowrap items-center">
            ...
            <div class="flex flex-col flex-shrink-0 space-y-1 w-auto md:flex-row md:space-y-0 md:space-x-3 items-center">
                <x-buttons.add-button :route="route('client-contacts.create')" gate="storeUpdate_client_contacts" :text="__('Add')" />
                <div class="flex items-center w-full space-x-3 hidden md:w-auto md:inline-block">
                    ...
                    <div id="actionsDropdown" class="z-10 hidden bg-white divide-y divide-gray-100 rounded shadow w-44 dark:bg-gray-600 dark:divide-gray-600">
                        <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="actionsDropdownButton">
                            <li>
                                <button type="button" onclick="location.href='{{ route('client-contacts.showUploadForm') }}'" class="relative w-full py-2 hover:bg-gray-100 dark:hover:bg-gray-500 dark:hover:text-white">
    ...
    <div class="relative bg-white dark:bg-gray-800 rounded-t-md md:w-auto md:ml-14 md:mr-2 m-auto shadow-md  dark:text-gray-900 mt-4">
        <div class="flex flex-col items-center justify-between p-4 space-y-3 md:flex-row md:space-y-0 md:space-x-4">
            <div class="w-full ">
                <form method="GET" action="{{ route('client-contacts.index') }}" id="search_form" class="flex items-center">
                    @csrf
                    <div class="flex flex-col md:flex-row w-full">
                        <div class="relative w-full">
                            ...
                            <input type="search" id="client_info" name="client_info" value="@if (isset($filters['client_info'])){{$filters['client_info']}} @endif" class="block w-full p-2 pl-10 text-sm text-gray-900 dark:text-white rounded bg-gray-100 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 border-gray-400 border focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150 placeholder:text-gray-400" placeholder="顧客№、顧客名">
                        </div>
            ...
            <tbody>
                @foreach ($clientContacts as $clientContact)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 text-gray-900 hover:bg-gray-200 dark:text-white dark:hover:bg-gray-600 clickable-row">
                        ...
                        <td class="pl-4 py-1 whitespace-nowrap">
                            <div class="flex items-center">
                                <input id="checkbox{{ $clientContact->id }}" type="checkbox" name="selectedIds[]" value="{{ $clientContact->id }}" form="bulkDeleteForm" class="checkbox-item  w-4 h-4 text-blue-600 bg-gray-100 border-gray-500 dark:border-white rounded border  focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-1 dark:bg-gray-700">
                            </div>
                        </td>
                        <td class="pl-4 py-1 whitespace-nowrap">
                            <button type="button" onclick="location.href='{{ route('client-contacts.edit', $clientContact) }}'"  class="button-edit-primary">
                                <div class="flex items-center">
                        ...
                        <td class="px-1 py-1 whitespace-nowrap">
                            {{ optional($clientContact->client)->client_num}}
                        </td>
                        <td class="px-1 py-1 whitespace-nowrap">
                            @if ($clientContact->client->is_dealer)
                                <div class="rounded bg-blue-400 w-14 px-2 py-1 font-semibold text-center text-xs">ディーラ</div>
                            @endif
                        </td>
                        <td class="px-1 py-1 whitespace-nowrap">
                            {{ optional($clientContact->client)->client_name}}
                        </td>

                        <td class="px-1 py-1 pt-3 whitespace-nowrap">
                            <ruby>
                            {{$clientContact->last_name}}<rt class="text-[9px]">{{$clientContact->last_name_kana}}</rt>
                            </ruby>
                            <ruby>
                                 {{$clientContact->first_name}}<rt class="text-[9px]">{{ $clientContact->first_name_kana }}</rt>
                            </ruby>
                        </td>
                        ...
                        <td class="px-1 py-1 whitespace-nowrap">
                            @if ($clientContact->is_billing_receiver)
                                <span class="bg-blue-100 text-blue-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-blue-400 border border-blue-400">
                                    請求先
                                </span>
                            @endif
                        </td>
                    @if($clientContact->is_retired == "1")
                    ...
                    {{-- 削除確認モーダル画面 Start --}}
                    <div id="deleteModal-{{$clientContact->id}}" tabindex="-1" class="fixed top-0 left-0 right-0 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
                        <div class="relative w-full max-w-md max-h-full">
                            <div class="relative bg-white rounded-s rounded-e shadow dark:bg-gray-700">
                                <button data-modal-hide="deleteModal-{{$clientContact->id}}" type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-s rounded-e text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white">
                                ...
                                <div class="p-6 text-center">
                                    ...
                                    <form action="{{route('client-contacts.destroy',$clientContact->id)}}" method="POST" class="text-center m-auto">
                                        @csrf
                                        @method('delete')
                                        <button type="submit" data-modal-hide="deleteModal-{{$clientContact->id}}" class="text-white bg-red-600 hover:bg-red-800 focus:ring-2 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-s rounded-e text-sm inline-flex items-center px-5 py-2.5 text-center mr-2">
                                            削除
                                        </button>
                                    </form>
                                    <button data-modal-hide="deleteModal-{{$clientContact->id}}" type="button" class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-2 focus:outline-none focus:ring-gray-200 rounded-s rounded-e border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">
                                        やっぱやめます
                                    </button>
                                </div>
                            </div>
