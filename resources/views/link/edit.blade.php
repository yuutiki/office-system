<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between">
            ...
            <div class="flex justify-end">
                <x-general-button onclick="location.href='{{route('link.index',$link)}}'">
                    戻る
                </x-general-button>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="mx-4 sm:p-8">
            <form method="post" action="{{route('link.update',$link)}}" enctype="multipart/form-data">
                @csrf
                @method('patch')
                ...
                <div class="mt-2 p-4 sm:p-4 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                    <div class="md:flex items-center">
                        ...
                        <input type="text" name="display_name" class="w-auto py-1 placeholder-gray-500 border border-gray-300 rounded-md mt-1" id="display_name" value="{{old('display_name',$link->display_name)}}">
                        </div>
                    </div>
                    @error('display_name')
                    ...
                    <div class="md:flex items-center">
                        ...
                        <input type="text" name="url" class="w-auto py-1 placeholder-gray-500 border border-gray-300 rounded-md mt-1" id="url" value="{{old('url',$link->url)}}">
                        </div>
                    </div>
                    @error('url')
                        <div class="text-red-500">{{$message}}</div>
                    ...
                    <div class="grid gap-4 mb-4 md:grid-cols-2 ">
                        ...
                        <div class="md:flex items-center">
                            ...
                            <input type="number" name="display_order" class="w-auto py-1 placeholder-gray-500 border border-gray-300 rounded-md mt-1" id="display_order" value="{{old('display_order',$link->display_order)}}">
                            </div>
                        </div>
                        @error('display_order')
                            <div class="text-red-500">{{$message}}</div>
