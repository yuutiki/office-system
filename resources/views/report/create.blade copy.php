...
{{-- 初期値で今日の日付突っ込む --}}
                    ...
                    <div class="container">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <h2 class="font-semibold text-gray-900 dark:text-white leading-none mt-4">報告先</h2>
                                <input type="text" class="border border-gray-300 rounded px-3 py-1.5 w-full mb-2" id="userSearch" placeholder="ユーザを検索...">
                                <h2 class="font-semibold text-gray-900 dark:text-white leading-none mt-4">報告先候補一覧</h2>
                                <ul class="border border-gray-300 rounded px-3 py-2 h-60 overflow-y-scroll " id="userList">
                                    @foreach ($users as $user)
                                        <li class=" border-b-2 text-sm cursor-pointer hover:bg-blue-100 p-1 dark:text-white" data-user-id="{{ $user->id }}">{{ $user->name }}</li>
                                    @endforeach
                                </ul>
                            </div>
                            <div>
                                <h2 class="text-lg font-semibold mb-4 mt-8 dark:text-white">報告先選択済み一覧</h2>
                                <ul class=" text-sm border border-gray-300 rounded px-3 py-2 h-60 overflow-y-scroll  mb-1 p-1 dark:text-white" id="selectedUserList">
                                </ul>
                            </div>
                        </div>
                    </div>
...
<style>
    /* 選択済みユーザーのLi要素に対してTailwind CSSのスタイルを適用 */
    #selectedUserList li {
        @apply bg-gray-100 p-1 my-1 rounded cursor-pointer border-b-2 hover:bg-blue-100;
    }
</style>
