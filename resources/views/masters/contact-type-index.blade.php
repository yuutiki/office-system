<x-app-layout>
                ...
                <div class="flex items-center space-x-3 w-auto hidden md:inline-block">
                    ...
                    {{-- <div id="actionsDropdown" class="z-10 hidden bg-white divide-y divide-gray-100 rounded shadow w-44 dark:bg-gray-600 dark:divide-gray-600">
                        <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="actionsDropdownButton">
                            <li>
                                @can('admin_contact_types')
                                    <button type="button" onclick="location.href='{{ route('contact-types.showUploadForm') }}'" class="relative w-full py-2 hover:bg-gray-100 dark:hover:bg-gray-500 dark:hover:text-white">
                            ...
                            <li>
                                @can('download_contact_types')
                                    <button type="button" onclick="location.href='{{ route('contact-types.downloadCsv', $filters ?? []) }}'" class="relative w-full items-center py-2 hover:bg-gray-100 dark:hover:bg-gray-500 dark:hover:text-white">
                                        <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
    ...
    <div class="relative bg-white dark:bg-gray-800 rounded-t-md md:w-auto md:ml-14 md:mr-2 m-auto shadow-md dark:text-gray-900 mt-4">
        <div class="relative bg-white dark:bg-gray-800">
            <div class="flex flex-col items-center justify-between p-4 space-y-3 md:flex-row md:space-y-0 md:space-x-4">
                <div class="w-full">
                    <form method="GET" action="{{ route('contact-type.index') }}" id="search_form" class="flex items-center">
                        @csrf
                ...
                @foreach ($contactTypes as $contactType)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-600 dark:text-white dark:hover:bg-gray-600">
                        ...
                        <td class="px-1 py-2 whitespace-nowrap text-gray-400">
                            {{ optional($contactType->updatedBy)->user_name }}
                        </td>
                        <td class="px-1 py-2 whitespace-nowrap text-gray-400">
                            <div class="hidden md:block">{{ $contactType->updated_at }}</div>
                            <div class="md:hidden">{{ $contactType->updated_at->format('Y-m-d')  }}</div>
                        </td>
                        <td class="px-1 py-1 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            <div class="text-center">
                                <button type="button" class="button-edit-primary" onclick="openDrawer('{{ $contactType->id }}')">
                                    <div class="flex items-center">
                    ...
                    <div id="drawer-{{ $contactType->id }}"
                        ...
                        <!-- フォーム部分 -->
                        <div class="">
                            <form method="POST" action="{{ route('contact-type.update', $contactType->id) }}" id="contact-type-form-{{ $contactType->id }}" data-is-changed="false">
                                @csrf
                                @method('PUT')

                                <div class="flex justify-between items-center p-4">
                                    <button type="button" data-drawer-id="{{ $contactType->id }}"
                                    ...
                                    <!-- 削除ボタン（モーダル表示トリガー） -->
                                    <button type="button" data-modal-target="deleteModal-{{ $contactType->id }}" data-modal-toggle="deleteModal-{{ $contactType->id }}" class="button-delete-primary rounded ">
                                        <div class="flex items-center px-3.5 py-1.5">
                                ...
                                <div class="space-y-4 p-4">
                                    ...
                                    <input type="hidden" name="updated_at" value="{{ $contactType->updated_at->format('Y-m-d H:i:s') }}">
                                    
                                    <div class="w-full">
                                        <label for="contact_type_code-{{ $contactType->id }}" class="block dark:text-gray-100 text-gray-900">
                                            コード
                                        </label>
                                        <input type="text" maxlength="2" name="contact_type_code" id="contact_type_code-{{ $contactType->id }}" 
                                                value="{{old('contact_type_code',$contactType->contact_type_code)}}" 
                                                class="input-readonly" 
                                                readonly>
                                        @error('contact_type_code')
                                            <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                                        @enderror
                                    </div>
                        
                                    <div class="w-full">
                                        <label for="contact_type_name-{{ $contactType->id }}" class="block dark:text-gray-100 text-gray-900">
                                            名称
                                        </label>
                                        <input type="text" maxlength="10" name="contact_type_name" id="contact_type_name-{{ $contactType->id }}" 
                                                value="{{ old('contact_type_name',$contactType->contact_type_name) }}" 
                                                class="input-secondary" 
                                                required>
                                        @error('contact_type_name')
                                            <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                                        @enderror
                                    </div>

                                    <div class="w-full" x-data="{ isActive: {{ old('is_active', $contactType->is_active) == 1 ? 'true' : 'false' }} }">
                                        ...
                                        <div class="flex items-center space-x-3">
                                            <!-- 四角いトグルボタン -->
                                            <label for="is_active-{{ $contactType->id }}" class="relative cursor-pointer">
                                                <input type="checkbox"name="is_active"id="is_active-{{ $contactType->id }}"
                                                    value="1"
                                                    x-model="isActive"
                                                    :checked="isActive"
                                                    class="sr-only peer">
                            ...
                            <!-- 削除確認モーダル -->
                            <div id="deleteModal-{{ $contactType->id }}" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                    ...
                                    <div class="relative bg-white rounded shadow dark:bg-gray-700">
                                        <div class="p-6 text-center">
                                            ...
                                            <div class="flex justify-center">
                                                <!-- 削除フォーム -->
                                                <form id="delete-form-{{ $contactType->id }}" action="{{ route('contact-type.destroy', $contactType->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" data-modal-hide="deleteModal-{{ $contactType->id }}" class="text-white bg-red-700 hover:bg-red-800 focus:outline-none font-medium rounded text-sm inline-flex items-center px-5 py-2.5 text-center mr-2 focus:ring-2 focus:ring-red-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800">
                                                        {{ __('delete') }}
                                                    </button>
                                                </form>
                                                <!-- キャンセルボタン -->
                                                <button data-modal-hide="deleteModal-{{ $contactType->id }}" type="button" class="text-gray-500 bg-white hover:bg-gray-100 focus:outline-none rounded border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-indigo-500 focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800">
                                                    キャンセル
                                                </button>
                                            </div>
                                        </div>
    ...
    <div id="drawer-create" class="drawer-component fixed top-0 right-0 z-50 h-screen overflow-y-auto bg-white dark:bg-gray-600 shadow-lg transform translate-x-full transition-transform duration-300 ease-in-out"
        ...
        <div class="p-4">
            <form method="POST" 
                action="{{ route('contact-type.store') }}" 
                id="contact-type-form-create" 
                ...
                <div class="space-y-4 mt-4">
                    <div class="w-full">
                        <label for="contact_type_code" class="block dark:text-gray-100 text-gray-900">
                            コード
                        </label>
                        <input type="text" maxlength="2" name="contact_type_code" id="contact_type_code"
                            value="{{ old('contact_type_code') }}"
                            class="input-secondary" 
                            required>
                        @error('contact_type_code')
                            <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                    ...
                    <div class="w-full">
                        <label for="contact_type_name" class="block dark:text-gray-100 text-gray-900">
                            名称
                        </label>
                        <input type="text" maxlength="10" name="contact_type_name" id="contact_type_name"
                            value="{{ old('contact_type_name') }}"
                            class="input-secondary" 
                            required>
                        @error('contact_type_name')
                            <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
    ...
    @push('scripts')
        <script>
            window.FORM_PREFIX = 'contact-type-form';
        </script>
