                ...
                @foreach ($contactTypes as $contactType)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-600 dark:text-white dark:hover:bg-gray-600">
                        ...
                        {{-- <td class="px-1 py-2 whitespace-nowrap">
                            {{$contactType->projectType_eng_name}}
                        </td> --}}
                        {{-- <td class="px-1 py-2 whitespace-nowrap">
                            {{$contactType->created_at}}
                        </td> --}}
                        {{-- <td class="px-1 py-2 whitespace-nowrap">
                            {{$contactType->created_by}}
                        </td> --}}
                        <td class="px-1 py-2 whitespace-nowrap">
                            {{-- {{$contactType->updatedBy->name}} --}}
                            {{ optional($contactType->updatedBy)->user_name }}
                        </td>
                        <td class="px-1 py-2 whitespace-nowrap">
                            {{$contactType->updated_at}}
                        </td>
                        <td class="px-1 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            <div class="text-center">
                                ...
                                編集
                                    ...
                                    <!-- drawer component -->
                    <div id="dupdateModal-{{$contactType->id}}" class="fixed top-0 right-0 z-50 h-screen p-4 overflow-y-auto transition-transform w-[30rem] translate-x-full bg-white  dark:bg-gray-800" tabindex="-1" aria-labelledby="dupdateModal-{{$contactType->id}}">
                        <div class="">
                            <h5 id="dupdateModal-{{$contactType->id}}" class="inline-flex items-center mb-4 font-semibold text-xl text-gray-500 dark:text-gray-400">
                                対応種別マスタ編集
                            </h5>
                            <button type="button" data-drawer-hide="dupdateModal-{{$contactType->id}}" aria-controls="dupdateModal-{{$contactType->id}}" class="text-gray-400 bg-transparent ml-8 hover:bg-gray-200 hover:text-gray-900 rounded-md text-sm w-8 h-8 absolute top-2.5 end-2.5 inline-flex items-center justify-center dark:hover:bg-gray-600 dark:hover:text-white" >
                                <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                                </svg>
                            </button>
                        </div>
                        <form method="POST" action="{{ route('contact-type.update', $contactType->id) }}">
                            @csrf
                            @method('PUT')
                            <div class="w-full flex flex-col col-span-2 mt-10">
                                <label for="contact_type_code-{{$contactType->id}}" class="font-semibold dark:text-gray-100 text-gray-900 leading-none mt-1">コード</label>
                                <input type="text" maxlength="2" name="contact_type_code" id="-{{$contactType->id}}" value="{{old('contact_type_code',$contactType->contact_type_code)}}" class="dark:bg-white w-auto py-1 border border-gray-300 rounded-s rounded-e mt-1 mb-1" required>
                            </div>
                            @error('contact_type_code')
                            <div class="text-red-500">{{ $message }}</div>
                          @enderror
                            <div class="w-full flex flex-col col-span-2">
                                <label for="contact_type_name-{{$contactType->id}}" class="font-semibold dark:text-gray-100 text-gray-900 leading-none mt-1">名称</label>
                                <input type="text" maxlength="10" name="contact_type_name" id="contact_type_name-{{$contactType->id}}" value="{{old('contact_type_name',$contactType->contact_type_name)}}" class="dark:bg-white w-auto py-1 border border-gray-300 rounded-s rounded-e mt-1 mb-1" required>
                            </div>
                            @error('contact_type_name')
                              <div class="text-red-500">{{ $message }}</div>
                            @enderror
                            {{-- <div class="w-full flex flex-col col-span-2">
                                <label for="product_type_eng_name-{{$contactType->id}}" class="font-semibold dark:text-gray-100 text-gray-900 leading-none mt-1">英名称</label>
                                <input type="text" name="product_type_eng_name" id="product_type_eng_name-{{$contactType->id}}" value="{{old('projectType_eng_name',$contactType->projectType_eng_name)}}" class="dark:bg-white w-auto py-1 border border-gray-300 rounded-s rounded-e mt-1 mb-1">
                            </div> --}}
