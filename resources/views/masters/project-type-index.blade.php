<x-app-layout>
    ...
    <div class="relative bg-white dark:bg-gray-800 rounded-t-md md:w-auto md:ml-14 md:mr-2 m-auto shadow-md dark:text-gray-900 mt-4">
        <div class="w-full dark:bg-gray-800 p-4">
            <form method="GET" action="{{ route('project-type.index') }}" id="search_form" class="flex flex-col md:flex-row md:items-center md:space-x-4 space-y-2 md:space-y-0">
                @csrf
                    ...
                    @foreach ($projectTypes as $projectType)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-200 dark:text-white dark:hover:bg-gray-600">
                            ...
                            <td class="px-1 py-2 whitespace-nowrap md:hidden">
                                {{$projectType->updated_at->format('Y-m-d')}}
                            </td>
                        </tr>
                        <!-- drawer component -->
                                ...
                                <form method="POST" action="{{ route('project-type.update', $projectType->id) }}" id="project-type-form-{{$projectType->id}}" data-is-changed="false">
                                    ...
                                    <div class="flex justify-between items-center p-4">
                                        ...
                                        <div class="flex gap-4">
                                            <x-buttons.save-button form-id="project-type-form-{{$projectType->id}}" id="saveButton" class="" data-drawer-id="{{$projectType->id}}">
                                                {{ __("update") }}
                                            </x-buttons.save-button>

                                            <button type="button" data-modal-target="deleteModal-{{$projectType->id}}" data-modal-show="deleteModal-{{$projectType->id}}" class="button-delete-primary">
                                    ...
                                    <div class="space-y-4 p-4">
                                        <div class="w-full">
                                            <label for="project_type_code-{{$projectType->id}}" class="block font-semibold dark:text-gray-100 text-gray-900">
                                                コード
                                            </label>
                                            <input type="text" 
                                                    maxlength="2" 
                                                    name="project_type_code" 
                                                    id="project_type_code-{{$projectType->id}}" 
                                        ...
                                        <div class="w-full">
                                            <label for="project_type_name-{{$projectType->id}}" class="block font-semibold dark:text-gray-100 text-gray-900">
                                                名称
                                            </label>
                                            <input type="text" 
                                                    maxlength="20" 
                                                    name="project_type_name" 
                                                    id="project_type_name-{{$projectType->id}}" 
                        ...
                        {{-- 削除確認モーダル画面 Start --}}
                        <div id="deleteModal-{{$projectType->id}}" tabindex="-1" class="fixed top-0 left-0 right-0 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
                            <div class="relative w-full max-w-md max-h-full">
                                <div class="relative bg-white rounded-s rounded-e shadow dark:bg-gray-700">
                                    <button data-modal-hide="deleteModal-{{$projectType->id}}" type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-s rounded-e text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white">
                                    ...
                                    <div class="p-6 text-center">
                                        ...
                                        <form action="{{route('project-type.destroy',$projectType->id)}}" method="POST" class="text-center m-auto">
                                            @csrf
                                            @method('delete')
                                            @can('delete_masters')
                                                <button type="submit" data-modal-hide="deleteModal-{{$projectType->id}}" class="text-white bg-red-600 hover:bg-red-800 focus:ring-2 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-s rounded-e text-sm inline-flex items-center px-5 py-2.5 text-center mr-2">
                                                    削除
                                                </button>
                                            @endcan
                                        </form>
                                        <button data-modal-hide="deleteModal-{{$projectType->id}}" type="button" class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-2 focus:outline-none focus:ring-gray-200 rounded-s rounded-e border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">
                                            やっぱやめます
                                        </button>
                                    </div>
                                </div>
        ...
        <div class="p-4">
            <form method="POST" 
                ...
                <div class="space-y-4 mt-4">
                    <div class="w-full">
                        <label for="project_type_code" class="block font-semibold dark:text-gray-100 text-gray-900">
                            コード
                        </label>
                        <input type="text" 
                            maxlength="2" 
                    ...
                    <div class="w-full">
                        <label for="project_type_name" class="block font-semibold dark:text-gray-100 text-gray-900">
                            名称
                        </label>
                        <input type="text" 
                            maxlength="20" 
