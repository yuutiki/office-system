<x-app-layout>
            ...
            <tbody>
                @foreach ($supportTimes as $supportTime)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-200 dark:text-white dark:hover:bg-gray-600">
                        ...
                        <td class="px-1 py-2 whitespace-nowrap text-gray-400">
                            <div class="hidden md:block">{{ $supportTime->updated_at }}</div>
                            <div class="md:hidden">{{ $supportTime->updated_at->format('Y-m-d')  }}</div>
                        </td>
                        ...
                        <div class="">
                            <form method="POST" action="{{ route('support-time.update', $supportTime->id) }}" id="support-time-form-{{ $supportTime->id }}" data-is-changed="false">
                                ...
                                <div class="flex justify-between items-center p-4">
                                    <button type="button" data-drawer-id="{{ $supportTime->id }}"
                                    ...
                                    <!-- 削除ボタン（モーダル表示トリガー） -->
                                    <button type="button" data-modal-target="deleteModal-{{ $supportTime->id }}" data-modal-toggle="deleteModal-{{ $supportTime->id }}" class="button-delete-primary rounded ">
                                        <div class="flex items-center px-3.5 py-1.5">
                            ...
                            <!-- 削除確認モーダル -->
                            <div id="deleteModal-{{ $supportTime->id }}" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                    ...
                                    <div class="relative bg-white rounded shadow dark:bg-gray-700">
                                        <div class="p-6 text-center">
                                            ...
                                            <div class="flex justify-center">
                                                <!-- 削除フォーム -->
                                                <form id="delete-form-{{ $supportTime->id }}" action="{{ route('support-time.destroy', $supportTime->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" data-modal-hide="deleteModal-{{ $supportTime->id }}" class="text-white bg-red-700 hover:bg-red-800 focus:outline-none font-medium rounded text-sm inline-flex items-center px-5 py-2.5 text-center mr-2 focus:ring-2 focus:ring-red-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800">
                                                        {{ __('delete') }}
                                                    </button>
                                                </form>
                                                <!-- キャンセルボタン -->
                                                <button data-modal-hide="deleteModal-{{ $supportTime->id }}" type="button" class="text-gray-500 bg-white hover:bg-gray-100 focus:outline-none rounded border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-indigo-500 focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800">
                                                    キャンセル
                                                </button>
                                            </div>
                                        </div>
            ...
            <form method="POST" 
                ...
                <div class="space-y-4 mt-4">
                    ...
                    <div class="w-full">
                        ...
                        <input type="text" maxlength="10" name="name" id="name"
                            value="{{ old('name') }}"
                            class="input-secondary" 
                            >
                        @error('name')
    ...
    @push('scripts')
        <script>
            window.FORM_PREFIX = 'support-time-form';
        </script>
