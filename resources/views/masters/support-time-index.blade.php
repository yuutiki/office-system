<x-app-layout>
                        ...
                        <form method="GET" action="{{ route('support-time.index') }}" id="search_form" class="flex items-center">
                            ...
                            <div class="flex flex-col md:flex-row w-full">
                                ...
                                <div class="relative w-full">
                                    ...
                                    <input type="search" id="time-code-search" name="code" value="@if (isset($timeCode)){{ $timeCode }}@endif" class="block w-full p-2 pl-10 text-sm text-gray-900 dark:text-white rounded bg-gray-100 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 border-gray-400 border focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150 placeholder:text-gray-400" placeholder="コード" >
                                </div>
                                ...
                                <div class="relative w-full mt-2 md:ml-2 md:mt-0">
                                    ...
                                    <input type="search" id="time-name-search" name="name" value="@if (isset($request->name)){{ $request->name }}@endif" class="block w-full p-2 pl-10 text-sm text-gray-900 dark:text-white rounded bg-gray-100 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 border-gray-400 border focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150 placeholder:text-gray-400" placeholder="名称" >
                                </div>
                    ...
                    @foreach ($supportTimes as $supportTime)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-600 dark:text-white dark:hover:bg-gray-600">
                            ...
                            <td class="px-1 py-2 whitespace-nowrap md:hidden">
                                {{$supportTime->updated_at->format('Y-m-d')}}
                            </td>
                            <td class="px-1 py-1 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                <div class="text-center">
                                    <button type="button" class="button-edit-primary" onclick="openDrawer('{{$supportTime->id}}')">
                                        <div class="flex items-center">
                        ...
                        <div id="drawer-{{$supportTime->id}}" 
                            ...
                            {{-- フォーム部分 --}}
                            <div class="">
                                <form method="POST" action="{{ route('support-time.update', $supportTime->id) }}" id="support-time-form-{{$supportTime->id}}" data-is-changed="false">
                                    @csrf
                                    @method('PUT')

                                    <div class="flex justify-between items-center p-4">
                                        <button type="button" 
                                            data-drawer-id="{{$supportTime->id}}"
                                    ...
                                    <div class="space-y-4 p-4">
                                        <div class="w-full">
                                            <label for="time_code-{{$supportTime->id}}" class="block font-semibold dark:text-gray-100 text-gray-900 mb-1">
                                                コード
                                            </label>
                                            <input type="text" 
                                                    maxlength="2" 
                                                    name="time_code" 
                                                    id="time_code-{{$supportTime->id}}" 
                                                    value="{{old('time_code',$supportTime->time_code)}}" 
                                        ...
                                        <div class="w-full">
                                            <label for="time_name-{{$supportTime->id}}" class="block font-semibold dark:text-gray-100 text-gray-900 mb-1">
                                                名称
                                            </label>
                                            <input type="text" 
                                                    maxlength="10" 
                                                    name="time_name" 
                                                    id="time_name-{{$supportTime->id}}" 
                                                    value="{{old('time_name',$supportTime->time_name)}}" 
    ...
    <div id="drawer-create" 
        ...
        <div class="p-4">
            <form method="POST" 
                action="{{ route('support-time.store') }}" 
                id="support-time-form-create" 
                ...
                <div class="space-y-4 mt-4">
                    <div class="w-full">
                        <label for="time_code" class="block font-semibold dark:text-gray-100 text-gray-900 mb-1">
                            コード
                        </label>
                        <input type="text" 
                            maxlength="2" 
                    ...
                    <div class="w-full">
                        <label for="time_name" class="block font-semibold dark:text-gray-100 text-gray-900 mb-1">
                            名称
                        </label>
                        <input type="text" 
                            maxlength="10" 
    ...
    {{-- オーバーレイ --}}
    {{-- <div id="overlay" class="fixed inset-0 bg-gray-900 bg-opacity-50 z-40 hidden opacity-0 transition-opacity duration-300"></div> --}}
    ...
    <script>
        ...
        const FormManager = {
            ...
            initializeCreateForm() {
                const form = document.getElementById('support-time-form-create');
                if (!form) return;
        ...
        function closeDrawer(id) {
            const drawer = document.getElementById(`drawer-${id}`);
            const overlay = document.getElementById('overlay');
            const form = document.getElementById(`support-time-form-${id}`);
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // フォームの初期化をシンプルに
            document.querySelectorAll('form[id^="support-time-form-"]').forEach(form => {
