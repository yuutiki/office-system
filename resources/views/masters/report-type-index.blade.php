<x-app-layout>
    ...
    <div class="relative bg-white dark:bg-gray-800 rounded-t-md md:w-auto md:ml-14 md:mr-2 m-auto shadow-md dark:text-gray-900 mt-4">
        <div class="w-full dark:bg-gray-800 p-4">
            <form method="GET" action="{{ route('report-type.index') }}" id="search_form" class="flex flex-col md:flex-row md:items-center md:space-x-4 space-y-2 md:space-y-0">
                @csrf
                ...
                <div class="relative w-full">
                    ...
                    <input type="search" name="name" value="{{ $typeName ?? '' }}" class="input-search" placeholder="名称">
                </div>
                ...
                <tbody>
                    @foreach ($reportTypes as $reportType)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-200 dark:text-white dark:hover:bg-gray-600">
                            ...
                            <td class="px-1 py-1 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                <div class="text-center">
                                    <button type="button" class="button-edit-primary" onclick="openDrawer('{{$reportType->id}}')">
                        ...
                        <div id="drawer-{{$reportType->id}}" 
                            ...
                            {{-- フォーム部分 --}}
                            <div class="">
                                <form method="POST" action="{{ route('report-type.update', $reportType->id) }}" id="report-type-form-{{$reportType->id}}" data-is-changed="false">
                                    @csrf
                                    @method('PUT')

                                    <div class="flex justify-between items-center p-4">
                                        <button type="button" 
                                            data-drawer-id="{{$reportType->id}}"
                                        ...
                                        <div class="flex gap-4">
                                            <x-buttons.save-button form-id="report-type-form-{{$reportType->id}}" id="saveButton" class="" data-drawer-id="{{$reportType->id}}">
                                                {{ __("update") }}
                                            </x-buttons.save-button>

                                            <button type="button" data-modal-target="deleteModal-{{$reportType->id}}" data-modal-show="deleteModal-{{$reportType->id}}" class="button-delete-primary">
                                    ...
                                    <div class="space-y-4 p-4">
                                        <div class="w-full">
                                            <label for="report_type_code-{{$reportType->id}}" class="block font-semibold dark:text-gray-100 text-gray-900">
                                                コード
                                            </label>
                                            <input type="text" 
                                                    maxlength="2" 
                                                    name="report_type_code" 
                                                    id="report_type_code-{{$reportType->id}}" 
                                                    value="{{old('report_type_code',$reportType->report_type_code)}}" 
                                                    class="input-readonly" 
                                                    readonly>
                                            @error('report_type_code')
                                                <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                                            @enderror
                                        </div>
                            
                                        <div class="w-full">
                                            <label for="report_type_name-{{$reportType->id}}" class="block font-semibold dark:text-gray-100 text-gray-900">
                                                名称
                                            </label>
                                            <input type="text" 
                                                    maxlength="20" 
                                                    name="report_type_name" 
                                                    id="report_type_name-{{$reportType->id}}" 
                                                    value="{{old('report_type_name',$reportType->report_type_name)}}" 
                                                    class="input-secondary" 
                                                    required>
                                            @error('report_type_name')
                                                <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                                            @enderror
                                        </div>
                                    </div>
                        ...
                        {{-- 削除確認モーダル画面 Start --}}
                        <div id="deleteModal-{{$reportType->id}}" tabindex="-1" class="fixed top-0 left-0 right-0 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
                            <div class="relative w-full max-w-md max-h-full">
                                <div class="relative bg-white rounded-s rounded-e shadow dark:bg-gray-700">
                                    <button data-modal-hide="deleteModal-{{$reportType->id}}" type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-s rounded-e text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white">
                                    ...
                                    <div class="p-6 text-center">
                                        ...
                                        <form action="{{route('report-type.destroy',$reportType->id)}}" method="POST" class="text-center m-auto">
                                            @csrf
                                            @method('delete')
                                            @can('delete_masters')
                                                <button type="submit" data-modal-hide="deleteModal-{{$reportType->id}}" class="text-white bg-red-600 hover:bg-red-800 focus:ring-2 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-s rounded-e text-sm inline-flex items-center px-5 py-2.5 text-center mr-2">
                                                    削除
                                                </button>
                                            @endcan
                                        </form>
                                        <button data-modal-hide="deleteModal-{{$reportType->id}}" type="button" class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-2 focus:outline-none focus:ring-gray-200 rounded-s rounded-e border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">
                                            やっぱやめます
                                        </button>
                                    </div>
                                </div>
        ...
        <div class="mt-1 mb-1 px-4">
            {{ $reportTypes->withQueryString()->links('vendor.pagination.custum-tailwind') }}  
        </div> 
    </div>
    ...
    <div id="drawer-create" 
        ...
        <div class="p-4">
            <form method="POST" 
                action="{{ route('report-type.store') }}" 
                id="report-type-form-create" 
                ...
                <div class="space-y-4 mt-4">
                    <div class="w-full">
                        <label for="report_type_code" class="block font-semibold dark:text-gray-100 text-gray-900">
                            コード
                        </label>
                        <input type="text" 
                            maxlength="2" 
                    ...
                    <div class="w-full">
                        <label for="report_type_name" class="block font-semibold dark:text-gray-100 text-gray-900">
                            名称
                        </label>
                        <input type="text" 
                            maxlength="20" 
    ...
    @push('scripts')
        <script>
            window.FORM_PREFIX = 'report-type-form';
        </script>
