<x-app-layout>
    ...
    <div class="w-5/6 m-auto relative overflow-x-auto rounded-s rounded-e mt-4 shadow-md dark:bg-gray-700 dark:text-gray-900 bg-gray-50">
                ...
                @foreach ($productTypes as $productType)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-600 dark:text-white dark:hover:bg-gray-600">
                        ...
                        {{-- <td class="px-1 py-2 whitespace-nowrap">
                            {{$productType->productType_eng_name}}
                        </td> --}}
                        {{-- <td class="px-1 py-2 whitespace-nowrap">
                        ...
                        <td class="px-1 py-2 whitespace-nowrap">
                            ...
                            {{ optional($productType->updatedBy)->user_name }}
                        </td>
                        <td class="px-1 py-2 whitespace-nowrap">
                        ...
                        <td class="px-1 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            <div class="text-center">
                                <button class="button-edit" type="button" data-drawer-target="dupdateModal-{{$productType->id}}" data-drawer-show="dupdateModal-{{$productType->id}}" data-drawer-placement="right" aria-controls="dupdateModal-{{$productType->id}}">
                                編集
                                </button>
                            </div>
                        </td>
                                    ...
                                    <!-- drawer component -->
                    <div id="dupdateModal-{{$productType->id}}" class="fixed top-0 right-0 z-50 h-screen p-4 overflow-y-auto transition-transform w-[30rem] translate-x-full bg-white  dark:bg-gray-800" tabindex="-1" aria-labelledby="dupdateModal-{{$productType->id}}">
                        <div class="">
                            <h5 id="dupdateModal-{{$productType->id}}" class="inline-flex items-center mb-4 font-semibold text-xl text-gray-500 dark:text-gray-400">
                                製品種別マスタ編集
                            </h5>
                            <button type="button" data-drawer-hide="dupdateModal-{{$productType->id}}" aria-controls="dupdateModal-{{$productType->id}}" class="text-gray-400 bg-transparent ml-8 hover:bg-gray-200 hover:text-gray-900 rounded-md text-sm w-8 h-8 absolute top-2.5 end-2.5 inline-flex items-center justify-center dark:hover:bg-gray-600 dark:hover:text-white" >
                        ...
                        <form method="POST" action="{{ route('product-type.update', $productType->id) }}">
                            ...
                            <div class="w-full flex flex-col col-span-2 mt-10">
                                <label for="product_type_code-{{$productType->id}}" class="font-semibold dark:text-gray-100 text-gray-900 leading-none mt-1">コード</label>
                                <input type="text" maxlength="2" name="type_code" class="dark:bg-white w-auto py-1 border border-gray-300 rounded-s rounded-e mt-1 mb-1" id="product_type_code-{{$productType->id}}" value="{{old('productType_code',$productType->type_code)}}" required>
                            </div>
                            <div class="w-full flex flex-col col-span-2">
                                <label for="product_type_name-{{$productType->id}}" class="font-semibold dark:text-gray-100 text-gray-900 leading-none mt-1">名称</label>
                                <input type="text" maxlength="10" name="type_name" class="dark:bg-white w-auto py-1 border border-gray-300 rounded-s rounded-e mt-1 mb-1" id="product_type_name-{{$productType->id}}" value="{{old('productType_name',$productType->type_name)}}" required>
                            </div>
                            {{-- <div class="w-full flex flex-col col-span-2">
                                <label for="product_type_eng_name-{{$productType->id}}" class="font-semibold dark:text-gray-100 text-gray-900 leading-none mt-1">英名称</label>
                                <input type="text" name="product_type_eng_name" class="dark:bg-white w-auto py-1 border border-gray-300 rounded-s rounded-e mt-1 mb-1" id="product_type_eng_name-{{$productType->id}}" value="{{old('productType_eng_name',$productType->productType_eng_name)}}">
                            </div> --}}
        ...
        {{-- {{ $productTypes->withQueryString()->links('vendor.pagination.custom-tailwind') }}   --}}
        </div> 
    </div>
</x-app-layout>