<x-app-layout>
    ...
    <div class="w-5/6 m-auto relative overflow-x-auto rounded-s rounded-e mt-4 shadow-md dark:bg-gray-700 dark:text-gray-900 bg-gray-50">
                ...
                @foreach ($productCategories as $productCategory)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-600 dark:text-white dark:hover:bg-gray-600">
                        ...
                        {{-- <td class="px-1 py-2 whitespace-nowrap">
                            {{$productCategory->projectType_eng_name}}
                        </td> --}}
                        {{-- <td class="px-1 py-2 whitespace-nowrap">
                        ...
                        <td class="px-1 py-2 whitespace-nowrap">
                            {{-- {{$productCategory->updatedBy->name}} --}}
                            {{ optional($productCategory->updatedBy)->name }}
                        </td>
                        <td class="px-1 py-2 whitespace-nowrap">
                        ...
                        <td class="px-1 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            <div class="text-center">
                                <button class="button-edit" type="button" data-drawer-target="dupdateModal-{{$productCategory->id}}" data-drawer-show="dupdateModal-{{$productCategory->id}}" data-drawer-placement="right" aria-controls="dupdateModal-{{$productCategory->id}}">
                                編集
                                </button>
                            </div>
                        </td>
                                    ...
                                    <!-- drawer component -->
                    <div id="dupdateModal-{{$productCategory->id}}" class="fixed top-0 right-0 z-50 h-screen p-4 overflow-y-auto transition-transform w-[30rem] translate-x-full bg-white  dark:bg-gray-800" tabindex="-1" aria-labelledby="dupdateModal-{{$productCategory->id}}">
                        <div class="">
                            <h5 id="dupdateModal-{{$productCategory->id}}" class="inline-flex items-center mb-4 font-semibold text-xl text-gray-500 dark:text-gray-400">
                                製品カテゴリマスタ編集
                            </h5>
                            <button type="button" data-drawer-hide="dupdateModal-{{$productCategory->id}}" aria-controls="dupdateModal-{{$productCategory->id}}" class="text-gray-400 bg-transparent ml-8 hover:bg-gray-200 hover:text-gray-900 rounded-md text-sm w-8 h-8 absolute top-2.5 end-2.5 inline-flex items-center justify-center dark:hover:bg-gray-600 dark:hover:text-white" >
                        ...
                        <form method="POST" action="{{ route('product-category.update', $productCategory->id) }}">
                            ...
                            <div class="w-full flex flex-col col-span-2 mt-10">
                                <label for="category_code-{{$productCategory->id}}" class="font-semibold dark:text-gray-100 text-gray-900 leading-none mt-1">コード</label>
                                <input type="text" maxlength="2" name="category_code" id="-{{$productCategory->id}}" value="{{old('category_code',$productCategory->category_code)}}" class="dark:bg-white w-auto py-1 border border-gray-300 rounded-s rounded-e mt-1 mb-1" required>
                            </div>
                            @error('category_code')
                            <div class="text-red-500">{{ $message }}</div>
                          @enderror
                            <div class="w-full flex flex-col col-span-2">
                                <label for="category_name-{{$productCategory->id}}" class="font-semibold dark:text-gray-100 text-gray-900 leading-none mt-1">名称</label>
                                <input type="text" maxlength="10" name="category_name" id="category_name-{{$productCategory->id}}" value="{{old('category_name',$productCategory->category_name)}}" class="dark:bg-white w-auto py-1 border border-gray-300 rounded-s rounded-e mt-1 mb-1" required>
                            </div>
                            @error('category_name')
                              <div class="text-red-500">{{ $message }}</div>
                            @enderror
                            {{-- <div class="w-full flex flex-col col-span-2">
                                <label for="product_type_eng_name-{{$productCategory->id}}" class="font-semibold dark:text-gray-100 text-gray-900 leading-none mt-1">英名称</label>
                                <input type="text" name="product_type_eng_name" id="product_type_eng_name-{{$productCategory->id}}" value="{{old('projectType_eng_name',$productCategory->projectType_eng_name)}}" class="dark:bg-white w-auto py-1 border border-gray-300 rounded-s rounded-e mt-1 mb-1">
                            </div> --}}
        ...
        {{ $productCategories->withQueryString()->links('vendor.pagination.custum-tailwind') }}  
        </div> 
    </div>
</x-app-layout>