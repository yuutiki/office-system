<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between">
            <h2 class="text-lg text-gray-900 dark:text-white flex">
                {{ Breadcrumbs::render('productSeriesMaster') }}
            </h2>
                ...
                <div class="flex items-center space-x-3 w-auto hidden md:inline-block">
                    ...
                    {{-- <div id="actionsDropdown" class="z-10 hidden bg-white divide-y divide-gray-100 rounded shadow w-44 dark:bg-gray-600 dark:divide-gray-600">
                        <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="actionsDropdownButton">
                            <li>
                                @can('admin_product_series')
                                    <button type="button" onclick="location.href='{{ route('product-series.showUploadForm') }}'" class="relative w-full py-2 hover:bg-gray-100 dark:hover:bg-gray-500 dark:hover:text-white">
                            ...
                            <li>
                                @can('download_product_series')
                                    <button type="button" onclick="location.href='{{ route('product-series.downloadCsv', $filters ?? []) }}'" class="relative w-full items-center py-2 hover:bg-gray-100 dark:hover:bg-gray-500 dark:hover:text-white">
                                        <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
    ...
    <div class="relative bg-white dark:bg-gray-800 rounded-t-md md:w-auto md:ml-14 md:mr-2 m-auto shadow-md dark:text-gray-900 mt-4">
        <div class="relative bg-white dark:bg-gray-800">
            <div class="flex flex-col items-center justify-between p-4 space-y-3 md:flex-row md:space-y-0 md:space-x-4">
                <div class="w-full">
                    <form method="GET" action="{{ route('product-series.index') }}" id="search_form" class="flex items-center">
                        @csrf
                        <div class="flex flex-col md:flex-row w-full">
                            ...
                            <div class="relative w-full">
                                ...
                                <input type="search" id="product-series-code-search" name="code" value="{{ request('code', '') }}" class="block w-full p-2 pl-10 text-sm text-gray-900 dark:text-white rounded bg-gray-100 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 border-gray-400 border focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150 placeholder:text-gray-400" placeholder="コード" >
                            </div>
                            ...
                            <div class="relative w-full mt-2 md:ml-2 md:mt-0">
                                ...
                                <input type="search" id="product-series-name-search" name="name" value="{{ request('name', '') }}" class="block w-full p-2 pl-10 text-sm text-gray-900 dark:text-white rounded bg-gray-100 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 border-gray-400 border focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150 placeholder:text-gray-400" placeholder="名称" >
                            </div>
    ...
    <div class="mt-1 mb-1 px-4 md:ml-9">
        {{ $productSeries->withQueryString()->links('vendor.pagination.custum-tailwind') }}  
    </div> 
    ...
    <div class="md:w-auto md:mr-2 md:ml-14 mb-4 relative overflow-x-auto rounded-b shadow-md dark:bg-gray-700 dark:text-gray-900 bg-gray-300">
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                @foreach ($productSeries as $productSeriesItem)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-600 dark:text-white dark:hover:bg-gray-600">
                        <td class="pl-8 py-2 whitespace-nowrap font-mono text-lg">
                            {{ $productSeriesItem->series_code }}
                        </td>
                        <td class="px-1 py-2 text-gray-900 whitespace-nowrap dark:text-white">
                            {{ $productSeriesItem->series_name }}
                        </td>
                        <td class="px-1 py-2 whitespace-nowrap">
                            @if($productSeriesItem->is_active)
                        ...
                        <td class="px-1 py-2 whitespace-nowrap text-gray-400">
                            {{ optional($productSeriesItem->updatedBy)->user_name }}
                        </td>
                        <td class="px-1 py-2 whitespace-nowrap text-gray-400">
                            <div class="hidden md:block">{{ $productSeriesItem->updated_at }}</div>
                            <div class="md:hidden">{{ $productSeriesItem->updated_at->format('Y-m-d')  }}</div>
                        </td>
                        <td class="px-1 py-1 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            <div class="text-center">
                                <button type="button" class="button-edit-primary" onclick="openDrawer('{{ $productSeriesItem->id }}')">
                                    <div class="flex items-center">
                    ...
                    <!-- drawer component -->
                    <div id="drawer-{{ $productSeriesItem->id }}"
                        ...
                        <!-- フォーム部分 -->
                        <div class="">
                            <form method="POST" action="{{ route('product-series.update', $productSeriesItem->id) }}" id="product-series-form-{{ $productSeriesItem->id }}" data-is-changed="false">
                                @csrf
                                @method('PUT')

                                <div class="flex justify-between items-center p-4">
                                    <button type="button" data-drawer-id="{{ $productSeriesItem->id }}"
                                    ...
                                    <!-- 削除ボタン（モーダル表示トリガー） -->
                                    <button type="button" data-modal-target="deleteModal-{{ $productSeriesItem->id }}" data-modal-toggle="deleteModal-{{ $productSeriesItem->id }}" class="button-delete-primary rounded ">
                                        <div class="flex items-center px-3.5 py-1.5">
                                ...
                                <div class="space-y-4 p-4">
                                    ...
                                    <input type="hidden" name="updated_at" value="{{ $productSeriesItem->updated_at->format('Y-m-d H:i:s') }}">
                                    
                                    <div class="w-full">
                                        <label for="series_code-{{ $productSeriesItem->id }}" class="block dark:text-gray-100 text-gray-900">
                                            コード
                                        </label>
                                        <input type="text" maxlength="4" name="series_code" id="series_code-{{ $productSeriesItem->id }}" 
                                                value="{{old('series_code',$productSeriesItem->series_code)}}" 
                                    ...
                                    <div class="w-full">
                                        <label for="series_name-{{ $productSeriesItem->id }}" class="block dark:text-gray-100 text-gray-900">
                                            名称
                                        </label>
                                        <input type="text" maxlength="20" name="series_name" id="series_name-{{ $productSeriesItem->id }}" 
                                                value="{{ old('series_name',$productSeriesItem->series_name) }}" 
                                    ...
                                    <div class="w-full" x-data="{ isActive: {{ old('is_active', $productSeriesItem->is_active) == 1 ? 'true' : 'false' }} }">
                                        ...
                                        <div class="flex items-center space-x-3">
                                            <!-- 四角いトグルボタン -->
                                            <label for="is_active-{{ $productSeriesItem->id }}" class="relative cursor-pointer">
                                                <input type="checkbox"name="is_active"id="is_active-{{ $productSeriesItem->id }}"
                                                    value="1"
                                                    x-model="isActive"
                                                    :checked="isActive"
                                                    class="sr-only peer">
                            ...
                            <!-- 削除確認モーダル -->
                            <div id="deleteModal-{{ $productSeriesItem->id }}" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                    ...
                                    <div class="relative bg-white rounded shadow dark:bg-gray-700">
                                        <div class="p-6 text-center">
                                            ...
                                            <div class="flex justify-center">
                                                <!-- 削除フォーム -->
                                                <form id="delete-form-{{ $productSeriesItem->id }}" action="{{ route('product-series.destroy', $productSeriesItem->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" data-modal-hide="deleteModal-{{ $productSeriesItem->id }}" class="text-white bg-red-700 hover:bg-red-800 focus:outline-none font-medium rounded text-sm inline-flex items-center px-5 py-2.5 text-center mr-2 focus:ring-2 focus:ring-red-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800">
                                                        {{ __('delete') }}
                                                    </button>
                                                </form>
                                                <!-- キャンセルボタン -->
                                                <button data-modal-hide="deleteModal-{{ $productSeriesItem->id }}" type="button" class="text-gray-500 bg-white hover:bg-gray-100 focus:outline-none rounded border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-indigo-500 focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800">
                                                    キャンセル
                                                </button>
                                            </div>
                                        </div>
    ...
    @if($productSeries && $productSeries->hasPages())
        <div class="mt-1 mb-1 px-4 md:ml-9">
            {{ $productSeries->withQueryString()->links('vendor.pagination.custum-tailwind') }}  
        </div> 
    @endif
    ...
    <div id="drawer-create" class="drawer-component fixed top-0 right-0 z-50 h-screen overflow-y-auto bg-white dark:bg-gray-600 shadow-lg transform translate-x-full transition-transform duration-300 ease-in-out"
        ...
        <div class="p-4">
            <form method="POST" 
                action="{{ route('product-series.store') }}" 
                id="product-series-form-create" 
                ...
                <div class="space-y-4 mt-4">
                    <div class="w-full">
                        <label for="series_code" class="block dark:text-gray-100 text-gray-900">
                            コード
                        </label>
                        <input type="text" maxlength="4" name="series_code" id="series_code"
                            value="{{ old('series_code') }}"
                            class="input-secondary" 
                            required>
                        @error('series_code')
                            <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                    ...
                    <div class="w-full">
                        <label for="series_name" class="block dark:text-gray-100 text-gray-900">
                            名称
                        </label>
                        <input type="text" maxlength="20" name="series_name" id="series_name"
    ...
    @push('scripts')
        <script>
            window.FORM_PREFIX = 'product-series-form';
        </script>
