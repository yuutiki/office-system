            ...
            <tbody>
                @foreach ($productSeries as $productSeries)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-600 dark:text-white dark:hover:bg-gray-600">
                        ...
                        <td class="pl-4 py-2 whitespace-nowrap">
                            {{ $productSeries->series_code }}
                        </td>
                        <td class="px-1 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            {{$productSeries->series_name}}
                        </td>
                        {{-- <td class="px-1 py-2 whitespace-nowrap">
                            {{$productSeries->productSeries_eng_name}}
                        </td> --}}
                        {{-- <td class="px-1 py-2 whitespace-nowrap">
                            {{$productSeries->created_at}}
                        </td> --}}
                        {{-- <td class="px-1 py-2 whitespace-nowrap">
                            {{$productSeries->created_by}}
                        </td> --}}
                        <td class="px-1 py-2 whitespace-nowrap">
                            {{-- {{$productSeries->updatedBy->name}} --}}
                            {{ optional($productSeries->updatedBy)->user_name }}
                        </td>
                        <td class="px-1 py-2 whitespace-nowrap">
                            {{$productSeries->updated_at}}
                        </td>
                        <td class="px-1 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            <div class="text-center">
                                <button class="button-edit" type="button" data-drawer-target="dupdateModal-{{$productSeries->id}}" data-drawer-show="dupdateModal-{{$productSeries->id}}" data-drawer-placement="right" aria-controls="dupdateModal-{{$productSeries->id}}">
                                編集
                                </button>
                            </div>
                        </td>
                                    ...
                                    <!-- drawer component -->
                    <div id="dupdateModal-{{$productSeries->id}}" class="fixed top-0 right-0 z-50 h-screen p-4 overflow-y-auto transition-transform w-[30rem] translate-x-full bg-white  dark:bg-gray-800" tabindex="-1" aria-labelledby="dupdateModal-{{$productSeries->id}}">
                        <div class="">
                            <h5 id="dupdateModal-{{$productSeries->id}}" class="inline-flex items-center mb-4 font-semibold text-xl text-gray-500 dark:text-gray-400">
                                製品シリーズマスタ編集
                            </h5>
                            <button type="button" data-drawer-hide="dupdateModal-{{$productSeries->id}}" aria-controls="dupdateModal-{{$productSeries->id}}" class="text-gray-400 bg-transparent ml-8 hover:bg-gray-200 hover:text-gray-900 rounded-md text-sm w-8 h-8 absolute top-2.5 end-2.5 inline-flex items-center justify-center dark:hover:bg-gray-600 dark:hover:text-white" >
                                <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                                </svg>
                            </button>
                        </div>
                        <form method="POST" action="{{ route('product-series.update', $productSeries->id) }}">
                            @csrf
                            @method('PUT')
                            <div class="w-full flex flex-col col-span-2 mt-10">
                                <label for="productSeries_code-{{$productSeries->id}}" class="font-semibold dark:text-gray-100 text-gray-900 leading-none mt-1">コード</label>
                                <input type="text" name="productSeries_code" class="dark:bg-white w-auto py-1 border border-gray-300 rounded-s rounded-e mt-1 mb-1" id="productSeries_code-{{$productSeries->id}}" value="{{old('productSeries_code',$productSeries->series_code)}}" required>
                            </div>
                            <div class="w-full flex flex-col col-span-2">
                                <label for="productSeries_name-{{$productSeries->id}}" class="font-semibold dark:text-gray-100 text-gray-900 leading-none mt-1">名称</label>
                                <input type="text" name="productSeries_name" class="dark:bg-white w-auto py-1 border border-gray-300 rounded-s rounded-e mt-1 mb-1" id="productSeries_name-{{$productSeries->id}}" value="{{old('productSeries_name',$productSeries->series_name)}}" required>
                            </div>
                            {{-- <div class="w-full flex flex-col col-span-2">
                                <label for="productSeries_eng_name-{{$productSeries->id}}" class="font-semibold dark:text-gray-100 text-gray-900 leading-none mt-1">英名称</label>
                                <input type="text" name="productSeries_eng_name" class="dark:bg-white w-auto py-1 border border-gray-300 rounded-s rounded-e mt-1 mb-1" id="productSeries_eng_name-{{$productSeries->id}}" value="{{old('productSeries_eng_name',$productSeries->productSeries_eng_name)}}">
                            </div> --}}
