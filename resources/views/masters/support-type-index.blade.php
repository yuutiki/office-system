<x-app-layout>
                ...
                <div class="flex items-center space-x-3 w-auto hidden md:inline-block">
                    ...
                    <div id="actionsDropdown" class="z-10 hidden bg-white divide-y divide-gray-100 rounded shadow w-44 dark:bg-gray-600 dark:divide-gray-600">
                        <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="actionsDropdownButton">
                            <li>
                                {{-- <x-buttons.csv-upload-button :route="route('support-types.showUploadForm')" gate="admin_masters" /> --}}
                            </li>
                            <li>
                            ...
                            <li>
                                @can('admin_masters')
                                    <button type="button" data-modal-target="deleteModal-all" data-modal-toggle="deleteModal-all" class="relative w-full flex items-center py-2 px-3 hover:bg-gray-100 dark:hover:bg-gray-500 dark:hover:text-white">
                    ...
                    <form method="GET" action="{{ route('support-type.index') }}" id="search_form" class="flex items-center">
                        ...
                        <div class="flex flex-col md:flex-row w-full">
                            ...
                            <div class="relative w-full">
                                ...
                                <input type="search" id="support-type-code-search" name="code" value="{{ request('code', '') }}" class="block w-full p-2 pl-10 text-sm text-gray-900 dark:text-white rounded bg-gray-100 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 border-gray-400 border focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150 placeholder:text-gray-400" placeholder="コード" >
                            </div>
                            ...
                            <div class="relative w-full mt-2 md:ml-2 md:mt-0">
                                ...
                                <input type="search" id="support-type-name-search" name="name" value="{{ request('name', '') }}" class="block w-full p-2 pl-10 text-sm text-gray-900 dark:text-white rounded bg-gray-100 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 border-gray-400 border focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150 placeholder:text-gray-400" placeholder="名称" >
                            </div>
    ...
    <div class="mt-1 mb-1 px-4 md:ml-9">
        {{ $supportTypes->withQueryString()->links('vendor.pagination.custom-tailwind') }}  
    </div> 
            ...
            <tbody>
                @foreach ($supportTypes as $supportType)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-200 dark:text-white dark:hover:bg-gray-600">
                        ...
                        <td class="px-1 py-2 whitespace-nowrap text-gray-400">
                            <div class="hidden md:block">{{ $supportType->updated_at }}</div>
                            <div class="md:hidden">{{ $supportType->updated_at->format('Y-m-d')  }}</div>
                        </td>
                        ...
                        <div class="">
                            <form method="POST" action="{{ route('support-type.update', $supportType->id) }}" id="support-type-form-{{ $supportType->id }}" data-is-changed="false">
                                ...
                                <div class="flex justify-between items-center p-4">
                                    <button type="button" data-drawer-id="{{ $supportType->id }}"
                                    ...
                                    <div class="flex gap-4">
                                        ...
                                        <!-- 削除ボタン（モーダル表示トリガー） -->
                                        <button type="button" data-modal-target="deleteModal-{{ $supportType->id }}" data-modal-toggle="deleteModal-{{ $supportType->id }}" class="button-delete-primary rounded ">
                                            <div class="flex items-center px-3.5 py-1.5">
                                ...
                                <div class="space-y-4 p-4">
                                    ...
                                    <input type="hidden" name="updated_at" value="{{ $supportType->updated_at->format('Y-m-d H:i:s') }}">
                                    
                                    <div class="w-full">
                                        <label for="code-{{ $supportType->id }}" class="block dark:text-gray-100 text-gray-900">
                                            サポート種別コード
                                        </label>
                                        <input type="text" maxlength="2" name="code" id="code-{{ $supportType->id }}" 
                                    ...
                                    <div class="w-full">
                                        <label for="name-{{ $supportType->id }}" class="block dark:text-gray-100 text-gray-900">
                                            サポート種別名称
                                        </label>
                                        <input type="text" maxlength="20" name="name" id="name-{{ $supportType->id }}" 
                            ...
                            <!-- 削除確認モーダル -->
                            <div id="deleteModal-{{ $supportType->id }}" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                    ...
                                    <div class="relative bg-white rounded shadow dark:bg-gray-700">
                                        <div class="p-6 text-center">
                                            ...
                                            <div class="flex justify-center">
                                                <!-- 削除フォーム -->
                                                <form id="delete-form-{{ $supportType->id }}" action="{{ route('support-type.destroy', $supportType->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" data-modal-hide="deleteModal-{{ $supportType->id }}" class="text-white bg-red-700 hover:bg-red-800 focus:outline-none font-medium rounded text-sm inline-flex items-center px-5 py-2.5 text-center mr-2 focus:ring-2 focus:ring-red-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800">
                                                        {{ __('delete') }}
                                                    </button>
                                                </form>
                                                <!-- キャンセルボタン -->
                                                <button data-modal-hide="deleteModal-{{ $supportType->id }}" type="button" class="text-gray-500 bg-white hover:bg-gray-100 focus:outline-none rounded border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-indigo-500 focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800">
                                                    キャンセル
                                                </button>
                                            </div>
                                        </div>
    ...
    @if($supportTypes && $supportTypes->hasPages())
        <div class="mt-1 mb-1 px-4 md:ml-9">
            {{ $supportTypes->withQueryString()->links('vendor.pagination.custom-tailwind') }}  
        </div> 
    @endif
        ...
        <div class="p-4">
            <form method="POST" 
                ...
                <div class="space-y-4 mt-4">
                    <div class="w-full">
                        <label for="code" class="block dark:text-gray-100 text-gray-900">
                            サポート種別コード
                        </label>
                        <input type="text" maxlength="2" name="code" id="code"
                            value="{{ old('code') }}"
                            class="input-secondary" 
                            required>
                        @error('code')
                    ...
                    <div class="w-full">
                        <label for="name" class="block dark:text-gray-100 text-gray-900">
                            サポート種別名称
                        </label>
    ...
    @push('scripts')
        <script>
            window.FORM_PREFIX = 'support-type-form';
        </script>
