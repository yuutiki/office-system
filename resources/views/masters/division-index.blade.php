<x-app-layout>
    ...
    <div class="w-5/6 m-auto relative overflow-x-auto rounded-s rounded-e mt-4 shadow-md dark:bg-gray-700 dark:text-gray-900 bg-gray-50">
            ...
            <tbody>
                @foreach ($divisions as $division)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-600 dark:text-white dark:hover:bg-gray-600">
                        ...
                        <td class="px-1 py-2 whitespace-nowrap">
                            {{-- {{$division->updatedBy->name}} --}}
                            {{ optional($division->updatedBy)->name }}
                        </td>
                        <td class="px-1 py-2 whitespace-nowrap">
                            {{$division->updated_at}}
                        </td>
                        <td class="px-1 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            <div class="text-center">
                                <button class="button-edit" type="button" data-drawer-target="dupdateModal-{{$division->id}}" data-drawer-show="dupdateModal-{{$division->id}}" data-drawer-placement="right" aria-controls="dupdateModal-{{$division->id}}">
                                編集
                                </button>
                            </div>
                        </td>
                                    ...
                                    <!-- drawer component -->
                    <div id="dupdateModal-{{$division->id}}" class="fixed top-0 right-0 z-50 h-screen p-4 overflow-y-auto transition-transform w-[30rem] translate-x-full bg-white  dark:bg-gray-800" tabindex="-1" aria-labelledby="dupdateModal-{{$division->id}}">
                        <div class="">
                            <h5 id="dupdateModal-{{$division->id}}" class="inline-flex items-center mb-4 font-semibold text-xl text-gray-500 dark:text-gray-400">
                              所属-部署マスタ編集
                            </h5>
                            <button type="button" data-drawer-hide="dupdateModal-{{$division->id}}" aria-controls="dupdateModal-{{$division->id}}" class="text-gray-400 bg-transparent ml-8 hover:bg-gray-200 hover:text-gray-900 rounded-md text-sm w-8 h-8 absolute top-2.5 end-2.5 inline-flex items-center justify-center dark:hover:bg-gray-600 dark:hover:text-white" >
                        ...
                        <form method="POST" action="{{ route('division.update', $division->id) }}">
                          ...
                          @method('PUT')
                            <div class="w-full flex flex-col col-span-2 mt-10">
                                <label for="division_code-{{$division->id}}" class="font-semibold dark:text-gray-100 text-gray-900 leading-none mt-1">コード</label>
                                <input type="text" maxlength="2" name="division_code" id="-{{$division->id}}" value="{{old('division_code',$division->division_code)}}" class="dark:bg-white w-auto py-1 border border-gray-300 rounded-s rounded-e mt-1 mb-1" required>
                            </div>
                            @error('division_code')
                            <div class="text-red-500">{{ $message }}</div>
                            @enderror
                            <div class="w-full flex flex-col col-span-2">
                                <label for="division_name-{{$division->id}}" class="font-semibold dark:text-gray-100 text-gray-900 leading-none mt-1">名称</label>
                                <input type="text" maxlength="100" name="division_name" id="division_name-{{$division->id}}" value="{{old('division_name',$division->division_name)}}" class="dark:bg-white w-auto py-1 border border-gray-300 rounded-s rounded-e mt-1 mb-1" required>
                            </div>
                            @error('division_name')
                              <div class="text-red-500">{{ $message }}</div>
                            @enderror
                            <div class="w-full flex flex-col col-span-2">
                              <label for="division_kana_name-{{$division->id}}" class="font-semibold dark:text-gray-100 text-gray-900 leading-none mt-1">カナ名称</label>
                              <input type="text" maxlength="100" name="division_kana_name" id="division_kana_name-{{$division->id}}" value="{{old('division_kana_name',$division->division_kana_name)}}" class="dark:bg-white w-auto py-1 border border-gray-300 rounded-s rounded-e mt-1 mb-1" required>
                            </div>
                            @error('division_kana_name')
                              <div class="text-red-500">{{ $message }}</div>
                            @enderror
                            <div class="w-full flex flex-col col-span-2">
                                <label for="division_eng_name-{{$division->id}}" class="font-semibold dark:text-gray-100 text-gray-900 leading-none mt-1">英名称</label>
                                <input type="text" maxlength="100" name="division_eng_name" id="division_eng_name-{{$division->id}}" value="{{old('division_eng_name',$division->division_eng_name)}}" class="dark:bg-white w-auto py-1 border border-gray-300 rounded-s rounded-e mt-1 mb-1" required>
                            </div>
                            @error('division_eng_name')
                              <div class="text-red-500">{{ $message }}</div>
                            @enderror
        ...
        {{ $divisions->withQueryString()->links('vendor.pagination.custum-tailwind') }}  
        </div> 
    </div>
</x-app-layout>