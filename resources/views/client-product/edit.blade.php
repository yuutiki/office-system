    ...
    <div class="max-w-7xl mx-auto px-2 md:pl-14">
        ...
        <div class="grid gap-4 mb-2 md:grid-cols-2">
            <div class="flex">
                <div class="w-full flex flex-col">
                    ...
                    <input type="text" form="createForm" name="client_num" tabindex="-1" class="input-secondary cursor-not-allowed" id="client_num" value="{{ $clientProduct->client->client_num }}" placeholder="顧客検索してください" >
                </div>
                <div hidden>
                    <label for="client_id">製品ID</label>
                    <input type="text" form="createForm" value="{{ old('client_id', $clientProduct->client_id) }}" name="client_id" id="client_id">
                </div>
            ...
            <div>
                <div class="">
                    ...
                    <input type="text" form="createForm" name="client_name" tabindex="-1" class="input-secondary cursor-not-allowed" id="client_name" value="{{ $clientProduct->client->client_name }}" readonly>
                </div>
                @error('client_id')
        ...
        <div class="hidden md:p-4 p-2 mb-4 rounded bg-gray-50 dark:bg-gray-800" id="profile" role="tabpanel" aria-labelledby="profile-tab">
            ...
            <div class="grid gap-4 mb-4 grid-cols-1 md:grid-cols-3 mt-2">
                    ...
                    <div class="w-full">
                        ...
                        <select form="createForm" id="product_series_id" name="product_series_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded focus:ring-primary-500 focus:border-primary-500 block w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500 cursor-not-allowed pointer-events-none" readonly>
                            ...
                            @foreach ($productSeries as $productSeries1)
                                <option value="{{ $productSeries1->id }}" @selected($productSeries1->id == old('product_series_id', $clientProduct->product->productSeries->id)) >
                ...
                <div>
                    ...
                    <select form="createForm" id="product_type_id" name="product_type_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded focus:ring-primary-500 focus:border-primary-500 block w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500 cursor-not-allowed pointer-events-none" readonly>
                        ...
                        <option value="{{ $productType->id }}" @selected($productType->id == old('product_type_id', $clientProduct->product->productType->id)) disabled>
                ...
                <div>
                    ...
                    <select form="createForm" id="product_split_type_id" name="product_split_type_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded focus:ring-primary-500 focus:border-primary-500 block w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500 cursor-not-allowed pointer-events-none" readonly>
                        ...
                        <option value="{{ $productSplitType->id }}" @selected($productSplitType->id == old('product_split_type_id', $clientProduct->product->productSplitType->id)) disabled>
                ...
                <div hidden>
                    ...
                    <input type="text" form="createForm" value="{{ old('product_id', $clientProduct->product_id) }}" name="product_id" id="product_id">
                </div>
                <div>
                    <div class="col-span-1 md:col-span-3">
                        <label for="product_name" class="block mb-1 text-sm text-gray-900 dark:text-white">製品名称</label>
                        <input type="text" form="createForm" name="product_name" id="product_name" value="{{ old('product_name', $clientProduct->product->product_name) }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded focus:ring-primary-600 focus:border-primary-600 block w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500 cursor-not-allowed" placeholder="製品検索をしてください" inert>
                    </div>
                    @error('product_id')
                        <div class="text-red-500">{{ $message }}</div>
                    @enderror
            ...
            <div class="grid gap-4 mb-4 md:grid-cols-4 mt-4">
                <div>
                    ...
                    <select form="createForm" id="product_version_id" name="product_version_id" tabindex="1" class="input-primary">
                        ...
                        @foreach ($productVersions as $productVersion)
                            <option value = "{{ $productVersion->id }}" @selected($productVersion->id == old('product_version_id', $clientProduct->productVersion->id)) >
                ...
                <div>
                    <label for="is_customized" class="block text-sm text-gray-900 dark:text-white">カスタマイズ</label>
                    <select form="createForm" id="is_customized" name="is_customized" tabindex="2" class="input-primary">
                        <option value="0" {{ old('is_customized', $clientProduct->is_customized ?? 0) == 0 ? 'selected' : '' }}>標準</option>
                        <option value="1" {{ old('is_customized', $clientProduct->is_customized ?? 0) == 1 ? 'selected' : '' }}>CUS</option>
                    </select>
                    @error('is_customized')
                        <div class="text-red-500">{{ $message }}</div>
                    @enderror
                ...
                <div>
                    ...
                    <select form="createForm" id="is_contracted" name="is_contracted" tabindex="3" class="input-primary">
                        <option value="0" {{ old('is_contracted', $clientProduct->is_contracted ?? 1) == 0 ? 'selected' : '' }}>未契約</option>
                        <option value="1" {{ old('is_contracted', $clientProduct->is_contracted ?? 1) == 1 ? 'selected' : '' }}>契約済</option>
                    </select>
                    @error('is_contracted')
                        <div class="text-red-500">{{ $message }}</div>
                    @enderror
                ...
                <div>
                    <div>
                        ...
                        <input type="number" form="createForm" min="1" tabindex="4" name="quantity" id="quantity" value="{{ old('quantity', $clientProduct->quantity) }}" class="input-secondary" required="">
                    </div>
                    @error('quantity')
                        <div class="text-red-500">{{ $message }}</div>
                    @enderror
                ...
                <div class="sm:col-span-4">
                    ...
                    <textarea id="install_memo" form="createForm" name="install_memo" tabindex="5" rows="4" class="input-primary">{{ old('install_memo', $clientProduct->install_memo) }}</textarea>
                </div>
                @error('install_memo')
                    <div class="text-red-500">{{ $message }}</div>
                @enderror
