<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between w-full whitespace-nowrap items-center">
            <h2 class="font-semibold text-lg text-gray-900 dark:text-white flex">
                {{ Breadcrumbs::render('clientpersons') }}
                <div class="ml-4">
                    {{ $count }}件
                </div>
            </h2>
            ...
            <div class="flex flex-col flex-shrink-0 space-y-1 w-auto md:flex-row md:space-y-0 md:space-x-3 items-center">
                <button type="button" onclick="location.href='{{ route('client-person.create') }}'" class="flex items-center pl-2 sm:px-4 py-2 text-sm font-medium text-white rounded bg-blue-700 hover:bg-blue-800 focus:ring-2 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
    ...
    <div class="relative bg-white dark:bg-gray-800 rounded-t-md md:w-auto md:ml-14 md:mr-2 m-auto shadow-md  dark:text-gray-900 mt-4">
        <div class="flex flex-col items-center justify-between p-4 space-y-3 md:flex-row md:space-y-0 md:space-x-4">
            <div class="w-full md:w-1/2">
                <form method="GET" action="{{ route('client-person.index') }}" id="search_form" class="flex items-center">
                    @csrf
                            ...
                            <div class="w-full md:ml-2">
                                ...
                                <div id="filterDropdown" class="z-50 hidden w-56 p-3 bg-gray-100 rounded-e rounded-s shadow dark:bg-gray-600">
                                    ...
                                    {{-- <ul class="space-y-2 text-sm" aria-labelledby="dropdownDefault">
                                        @foreach ($clientPersonsTypes as $clientPersonsType)
                                        <li class="flex items-center">
                                            <input id="clientPersonsType-{{ $clientPersonsType->id }}" type="checkbox" name="clientPersons_types[]" @if(in_array($clientPersonsType->id, $selectedclientPersonsTypes)) checked @endif value="{{$clientPersonsType->id}}" class="w-4 h-4 bg-gray-100 border-gray-300 rounded text-blue-600 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500" />
                                            <label for="clientPersonsType-{{ $clientPersonsType->id }}" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-100">{{ $clientPersonsType->type_name }}</label>
                                        </li>                       
                                        @endforeach
                                    </ul>
                ...
                {{-- <div class="flex flex-col items-stretch justify-end flex-shrink-0 w-full space-y-2 md:w-auto md:flex-row md:space-y-0 md:items-center md:space-x-3">
                    <button type="button" onclick="location.href='{{ route('client-person.create') }}'" class="flex items-center justify-center px-4 py-2 text-sm font-medium text-white rounded-s rounded-e bg-blue-700 hover:bg-blue-800 focus:ring-2 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
    ...
    <div class="md:w-auto md:ml-14 md:mr-2 relative overflow-x-auto rounded-b shadow-md dark:bg-gray-700  dark:text-gray-900 bg-gray-300">
            ...
            <tbody>
                @foreach ($clientPersons as $clientPerson)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 text-gray-900 hover:bg-gray-200 dark:text-white dark:hover:bg-gray-600 clickable-row">
                        ...
                        <td class="pl-4 py-1 whitespace-nowrap">
                            <button type="button" onclick="location.href='{{ route('client-person.edit', $clientPerson) }}'"  class="button-edit-primary">
                                <div class="flex items-center">
                    ...
                    {{-- 削除確認モーダル画面 Start --}}
                    <div id="deleteModal-{{$clientPerson->id}}" tabindex="-1" class="fixed top-0 left-0 right-0 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
                        <div class="relative w-full max-w-md max-h-full">
                            <div class="relative bg-white rounded-s rounded-e shadow dark:bg-gray-700">
                                <button data-modal-hide="deleteModal-{{$clientPerson->id}}" type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-s rounded-e text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white">
                                ...
                                <div class="p-6 text-center">
                                    ...
                                    <form action="{{route('client-person.destroy',$clientPerson->id)}}" method="POST" class="text-center m-auto">
                                        @csrf
                                        @method('delete')
                                        <button type="submit" data-modal-hide="deleteModal-{{$clientPerson->id}}" class="text-white bg-red-600 hover:bg-red-800 focus:ring-2 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-s rounded-e text-sm inline-flex items-center px-5 py-2.5 text-center mr-2">
                                            削除
                                        </button>
                                    </form>
                                    <button data-modal-hide="deleteModal-{{$clientPerson->id}}" type="button" class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-2 focus:outline-none focus:ring-gray-200 rounded-s rounded-e border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">
                                        やっぱやめます
                                    </button>
                                </div>
                            </div>
                    ...
                    <!-- 更新drawer --> 
                    {{-- <div id="dupdateModal-{{$clientPerson->id}}" class="fixed top-0 right-0 z-50 h-screen p-4 overflow-y-auto transition-transform md:w-1/2 translate-x-full bg-gray-200 dark:bg-gray-800" tabindex="-1" aria-labelledby="dupdateModal-{{$clientPerson->id}}">
                        <div class="">
                            <h5 id="dupdateModal-{{$clientPerson->id}}" class="inline-flex items-center mb-4 font-semibold text-xl text-gray-500 dark:text-gray-400">
                                サポート詳細
                                -{{ $clientPerson->title }}
                            </h5>
                            <button type="button" data-drawer-hide="dupdateModal-{{$clientPerson->id}}" aria-controls="dupdateModal-{{$clientPerson->id}}" class="text-gray-400 bg-transparent ml-8 hover:bg-gray-200 hover:text-gray-900 rounded-md text-sm w-8 h-8 absolute top-2.5 end-2.5 inline-flex items-center justify-center dark:hover:bg-gray-600 dark:hover:text-white" >
                                <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                                </svg>
                            </button>
                        </div>
                        <form id="updateForm-{{$clientPerson->id}}" method="POST" action="{{ route('clientPerson.update', $clientPerson->id) }}">
                            @csrf
                            @method('PUT')

                            <label class="relative inline-flex items-center cursor-pointer mt-4">
                                <input type="hidden" name="is_enabled_{{$clientPerson->id}}" value="0">
                                <input type="checkbox" name="is_enabled_{{$clientPerson->id}}" id="is_enabled-{{$clientPerson->id}}" value="1" class="sr-only peer" {{ old('is_enabled_' . $clientPerson->id, $clientPerson->is_enabled) == 1 ? 'checked' : '' }}>
                            ...
                            <div class="grid  gap-4 my-4 md:grid-cols-4">
                                <div class="relative z-0">
                                    <input type="text" id="client_num" name="client_num" value="{{ $clientPerson->client->client_num }}" class="block py-2.5 px-0 w-full text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " readonly />
                                ...
                                <div class="relative z-0">
                                    <input type="text" id="client_name" name="client_name" value="{{ $clientPerson->client->client_name }}" class="block py-2.5 px-0 w-full text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " readonly />
                                ...
                                <div class="relative z-0">
                                    <input type="text" id="sales_person" name="sales_person" value="{{ $clientPerson->client->user->name }}" class="block py-2.5 px-0 w-full text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " readonly />
                                ...
                                <div class="relative z-0">
                                    <input type="text" id="department_id" name="department_id" value="{{ $clientPerson->client->department->department_name }}" class="block py-2.5 px-0 w-full text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " readonly />
                            ...
                            <div class="w-full flex flex-col col-span-2 mt-4">
                                <label for="title-{{$clientPerson->id}}" class="font-semibold dark:text-gray-100 text-gray-900 leading-none mt-1">表題</label>
                                <input type="text" maxlength="100" name="title_{{$clientPerson->id}}" id="title-{{$clientPerson->id}}" value="{{old('title' . $clientPerson->id, $clientPerson->title)}}" class="dark:bg-white w-auto py-1 border border-gray-300 rounded-s rounded-e mt-1 mb-1" required>
                            </div>
                            @error('title_' . $clientPerson->id)
                                <div class="text-red-500">{{ $message }}</div>
                            @enderror

                            <div class="grid gap-4 my-4 md:grid-cols-4">
                                <div class="w-full flex flex-col">
                                    <label for="received_at-{{$clientPerson->id}}" class="font-semibold dark:text-gray-100 text-gray-900 leading-none mt-1">受付日</label>
                                    <input type="date" maxlength="20" name="received_at_{{$clientPerson->id}}" id="received_at-{{$clientPerson->id}}" value="{{ old('received_at_' . $clientPerson->id, $clientPerson->received_at) }}" class="dark:bg-white w-auto py-1 border border-gray-300 rounded-s rounded-e mt-1 mb-1" required>
                                </div>
                                @error('received_at_' . $clientPerson->id)
                                    <div class="text-red-500">{{ $message }}</div>
                                @enderror
                            </div>

                            <div class="w-full flex flex-col">
                                <label for="request_content-{{$clientPerson->id}}" class="font-semibold dark:text-gray-100 text-gray-900 leading-none mt-4">内容</label>
                                <textarea name="request_content_{{$clientPerson->id}}" id="request_content-{{$clientPerson->id}}" class="w-auto py-1 border text-sm border-gray-300 rounded-md mt-1 placeholder-gray-400" data-auto-resize="true"  cols="30" rows="8">{{ old('request_content_' . $clientPerson->id , $clientPerson->request_content) }}</textarea>
                                @error('request_content_' . $clientPerson->id)
                                    <div class="text-red-500">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="grid gap-4 my-4 md:grid-cols-2">
                                <div class="w-full flex flex-col">
                                    <label for="clientPerson_type_id" class="block font-medium text-gray-900 dark:text-white">サポート種別</label>
                                    <select name="clientPerson_type_id_{{$clientPerson->id}}" id="clientPerson_type_id-{{$clientPerson->id}}" value="{{old('clientPerson_type_id')}}" class="bg-gray-50 border border-gray-300 text-gray-900 rounded-s rounded-e focus:ring-primary-600 focus:border-primary-600 block w-full py-1 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
                                        @foreach($clientPersonTypes as $clientPersonType)
                                        <option value="{{ $clientPersonType->id }}"  @selected($clientPersonType->id == $clientPerson->clientPerson_type_id)>{{ $clientPersonType->type_name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                @error('clientPerson_type_id_' . $clientPerson->id)
                                    <div class="text-red-500">{{$message}}</div>
                                @enderror
                            </div>
                            ...
                            <ul class=" mt-4 items-center w-full text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded sm:flex dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                                <li class="w-full border-b border-gray-200 sm:border-b-0 sm:border-r dark:border-gray-600">
                                    <div class="flex items-center pl-3">
                                        <input id="is_finished_{{ $clientPerson->id }}" name="is_finished_{{ $clientPerson->id }}" type="hidden" value="0" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
                                        @if($clientPerson->is_finished)
                                            <input id="is_finished_{{ $clientPerson->id }}" name="is_finished_{{ $clientPerson->id }}" type="checkbox" value="1" checked="checked" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
                                        @else
                                            <input id="is_finished_{{ $clientPerson->id }}" name="is_finished_{{ $clientPerson->id }}" type="checkbox" value="1" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
                                        @endif
                                        <label for="is_finished_{{ $clientPerson->id }}" class="w-full py-3 ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">対応完了済</label>
                                    </div>
                                    @error('is_finished_' . $clientPerson->id)
                                     <div class="text-red-500">{{ $message }}</div>
                                    @enderror
                                </li>
                                <li class="w-full border-b border-gray-200 sm:border-b-0 sm:border-r dark:border-gray-600">
                                    <div class="flex items-center pl-3">
                                        <input id="is_faq_target1_{{ $clientPerson->id }}" name="is_faq_target_{{ $clientPerson->id }}" type="hidden" value="0" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
                                        @if($clientPerson->is_faq_target === 1)
                                            <input id="is_faq_target_{{ $clientPerson->id }}" name="is_faq_target_{{ $clientPerson->id }}" type="checkbox" checked="checked" value="1" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
                                        @else
                                            <input id="is_faq_target_{{ $clientPerson->id }}" name="is_faq_target_{{ $clientPerson->id }}" type="checkbox" value="1" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
                                        @endif
                                        <label for="is_faq_target_{{ $clientPerson->id }}" class="w-full py-3 ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">FAQ対象</label>
                                    </div>
                                    @error('is_faq_target_' . $clientPerson->id)
                                     <div class="text-red-500">{{ $message }}</div>
                                    @enderror
                                </li>
                            </ul>
                            <div class="grid grid-cols-2 gap-4 mt-4">
                                <button type="button" onclick="submitAndUpdateDrawer({{$clientPerson->id}})" class="w-full justify-center text-white bg-blue-700 hover:bg-blue-800 focus:ring-2 focus:outline-none focus:ring-blue-300 font-medium rounded-s rounded-e text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                                    {{ __('Update') }}
                                </button>
                                <button type="button" data-modal-target="deleteModal-{{$clientPerson->id}}" data-modal-show="deleteModal-{{$clientPerson->id}}" class="w-full justify-center text-red-600 inline-flex items-center hover:text-white border border-red-600 hover:bg-red-600 focus:ring-2 focus:outline-none focus:ring-red-300 font-medium rounded-s rounded-e text-sm px-5 py-2.5 text-center dark:border-red-500 dark:text-red-500 dark:hover:text-white dark:hover:bg-red-600 dark:focus:ring-red-900">
                                    <svg aria-hidden="true" class="w-5 h-5 mr-1 -ml-1" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clip-rule="evenodd"></path></svg>
                                    {{ __('Delete') }}
                                </button>
                            </div>
        ...
        {{ $clientPersons->withQueryString()->links('vendor.pagination.custum-tailwind') }}  
        </div> 
    </div>
    ...
    <div class="relative w-full max-w-2xl max-h-full">
        ...
        <div class="relative bg-white rounded-s rounded-e shadow dark:bg-gray-700">
            ...
            <div class="p-6 space-y-6 mr-20 mt-4">
                {{-- <form action="{{ route('clientPersons.upload') }}" method="POST" enctype="multipart/form-data" class="flex items-center" id="csv_form1"> --}}
                    @csrf
...
<script>
    function submitAndUpdateDrawer(clientPersonsId) {
        // 保存処理（ここではLocalStorageを使用）
        localStorage.setItem('updateDrawerId', clientPersonsId);

        // フォームのsubmit
        document.getElementById('updateForm-' + clientPersonsId).submit();
    }
</script>
