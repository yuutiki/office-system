<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="mx-4 sm:p-8">
            <form id="form1" method="post" action="{{route('project.update', $project)}}" enctype="multipart/form-data" autocomplete="new-password">
                @csrf
                <!-- 顧客検索ボタン -->
                ...
                <div class="grid gap-4 mt-6 mb-4 sm:grid-cols-3">
                    ...
                    <div class="hidden">
                        ...
                        <input type="text" name="client_num" id="client_num" value="{{old('client_num',$project->client->client_num)}}" class="dark:bg-gray-400 w-full py-1 placeholder-gray-400 border border-gray-300 rounded-md mt-1 ">
                    </div>
                    <div class="">
                    ...
                    <div class="">
                        ...
                        <input type="text" name="project_num" class="dark:bg-gray-400 w-full py-1 placeholder-gray-400 border border-gray-300 rounded-md mt-1" id="project_num" value="{{old('project_num',$project->project_num)}}" placeholder="登録時に自動採番されます" readonly>
                        @error('project_num')
                            <div class="text-red-500">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-span-2">
                        ...
                        <input type="text" name="project_name" class="w-full py-1 placeholder-gray-400 border border-gray-300 rounded-md mt-1" id="project_name" value="{{old('project_name',$project->project_name)}}" placeholder="">
                        @error('project_name')
                            <div class="text-red-500">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="w-full relative overflow-x-auto shadow-md rounded-lg mx-auto mt-1 boeder-2 bg-gray-300 dark:bg-gray-700">
                        <table class="w-full text-sm font-medium text-left text-gray-800 dark:text-gray-400">
                            ...
                            @foreach ($revenuesWithPeriod as $projectRevenue)
                                ...
                                {{-- 削除確認モーダル画面 Start --}}
                                <div id="deleteModal-{{$project->id}}" tabindex="-1" class="fixed top-0 left-0 right-0 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full animate-slide-in-top">
                        ...
                        <div class="grid gap-4 mt-1 mb-4 sm:grid-cols-5">
                            ...
                            <div class="w-full flex flex-col hidden">
                                ...
                                <input type="text" name="billing_corporation_id" class="w-auto py-1 border border-gray-300 rounded-md mt-1 mb-2" id="billing_corporation_id" value="{{old('billing_corporation_id',$project->billing_corporation_id)}}" placeholder="">
                            </div>
                            <div class="w-full flex flex-col">
                        ...
                        <div class="grid gap-4 mt-6 mb-4 sm:grid-cols-5">
                            <div class="w-full flex flex-col">
                                ...
                                <input type="month" min="1900-01" max="2100-12" name="proposed_order_date" value="{{ old('proposed_order_date', \Carbon\Carbon::parse($project->proposed_order_date)->format('Y-m') ?? '') }}" class="w-auto py-1 border border-gray-300 rounded-md mt-1" required>
                            </div>
                            <div class="w-full flex flex-col">
                                <label for="proposed_delivery_date" class="font-semibold dark:text-gray-100 text-gray-900 leading-none mt-2">納品予定月</label>
                                <input type="month" min="1900-01" max="2100-12"  name="proposed_delivery_date" value="{{ old('proposed_delivery_date', \Carbon\Carbon::parse($project->proposed_delivery_date)->format('Y-m') ?? '') }}" class="w-auto py-1 border border-gray-300 rounded-md mt-1" required>
                            </div>
                            <div class="w-full flex flex-col">
                                <label for="proposed_accounting_date" class="font-semibold dark:text-gray-100 text-gray-900 leading-none mt-2">検収/計上予定月</label>
                                <input type="month" min="1900-01" max="2100-12"  name="proposed_accounting_date" value="{{ old('proposed_accounting_date', \Carbon\Carbon::parse($project->proposed_accounting_date)->format('Y-m') ?? '') }}" class="w-auto py-1 border border-gray-300 rounded-md mt-1" required>
                            </div>
                            <div class="w-full flex flex-col">
                                <label for="proposed_payment_date" class="font-semibold dark:text-gray-100 text-gray-900 leading-none mt-2">入金予定月</label>
                                <input type="month" min="1900-01" max="2100-12"  name="proposed_payment_date" value="{{ old('proposed_payment_date', \Carbon\Carbon::parse($project->proposed_payment_date)->format('Y-m') ?? '') }}" class="w-auto py-1 border border-gray-300 rounded-md mt-1" required>
                            </div>
                        </div>
                        <div class="w-full flex flex-col">
                            <label for="project_memo" class="font-semibold dark:text-gray-100 text-gray-900 leading-none mt-4">プロジェクト備考</label>
                            <textarea name="project_memo" class="w-auto py-1 border border-gray-300 rounded-md mt-1" id="project_memo" value="{{old('project_memo',$project->project_memo)}}" cols="30" rows="5">{{old('project_memo',$project->project_memo)}}</textarea>
                        </div>
