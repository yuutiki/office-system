...
const ProductSearchModal = {
    ...
    search: async function(modalId, screenId) {
        const modal = document.getElementById(modalId);
        const searchData = {
            product_name: modal.querySelector(`#${modalId}_product_name`).value,
            product_series_id: modal.querySelector(`#${modalId}_product_series_id`).value,
            product_type_id: modal.querySelector(`#${modalId}_product_type_id`).value,
            product_split_type_id: modal.querySelector(`#${modalId}_product_split_type_id`).value,
            screen_id: screenId
        };
    ...
    setLoadingState: function(modalId, isLoading) {
        const searchButton = document.querySelector(`#${modalId} button[onclick*="ProductSearchModal.search"]`);
    ...
    updateResults: function(modalId, results, displayItems) {
        ...
        results.forEach(result => {
            ...
            row.setAttribute('aria-label', `${result.product_name} を選択`);

            row.innerHTML = displayItems.map(item => {
                const cellClasses = [
                    this.CLASSES.CELL,
                    item.column_key === 'product_name' ? this.CLASSES.CELL_WIDE : '',
                    item.column_key !== 'product_name' ? this.CLASSES.CELL_NOWRAP : ''
                ].filter(Boolean).join(' ');
